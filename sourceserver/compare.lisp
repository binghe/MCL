...
;; cancel button claims it is
;; ----- 5.1b2
;;  4 1/30/95  akh  probably none
...
(defun buffer-next-line (bstream)
  (let ((a-line (read-line (bstream-stream bstream) nil :eof)))
    (unless (eq a-line :eof)
...
(defun get-file-difs (file1 file2 &optional difs-window?)
    ;; First check the files exist or not, then open them.  Hai 8/2/93
    (if (and (probe-file file1) (probe-file file2))
         (with-open-file (s1 file1)
              (with-open-file (s2 file2)
         ...
         (progn
             (message-dialog (format nil "Files do not exist.~&File1 is ~a,~&File2 is ~a"
                                                       (probe-file file1)
                                                       (probe-file file2))
                                         :size #@(300 200))
             :no-exist
             ;(copy-file file1 file2 :if-exists nil)
             )))
...
(defparameter *compare-prefs-loaded* nil)
...
; this sucks if you move a saved application from one machine to another.
  ...
  (let* ((table (view-named 'the-table w))
    ...
    (if cell
      (let* ((short-file (cell-contents table cell))
        ...
        (eval-enqueue `(compare-files ,from-file ,to-file
                                    ,difs?
                                    ,#'(lambda ()
...
(defmethod generate-file-list ((w compare-directory-dialog))
  (with-cursor *watch-cursor*
    (let* ((from-dir (dialog-item-text (view-named 'from-dir-text w)))
           (file-list (mapcar #'(lambda (pathname)
                                  (subseq (mac-namestring pathname) (1- (length from-dir))))
                              (directory (merge-pathnames ":**:*.*" from-dir))))
           (compare-button (view-named 'compare-button w)))
      (setf (from-dir w) from-dir
