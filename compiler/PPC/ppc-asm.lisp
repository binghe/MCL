...
;; $Log: ppc-asm.lisp,v $
;; Revision 1.8  2003/12/08 08:24:29  gtbyers
;; No more #-ANSI-MAKE-LOAD-FORM.  Define load-forms later, to ease
;; cross-compilation.
;;
;; Revision 1.7  2003/12/01 17:56:05  gtbyers
;; recover pre-MOP changes
;;
;; Revision 1.5  2003/11/17 06:22:31  gtbyers
;; handle 5-bit vector instruction immediates correctly.
;;
;; Revision 1.4  2002/11/28 01:30:26  gtbyers
;; $ppc-operand-vector -> $ppc-operand-vr
;;
;; Revision 1.3  2002/11/25 05:36:34  gtbyers
;; Define vector operand types and vector instructions.
;;
;; Revision 1.2  2002/11/18 05:36:17  gtbyers
;; Add CVS log marker
;;
;;	Change History (most recent first):
;;  6 12/14/95 akh  don't eval-when compile-toplevel of defstructs
;;  (do not edit before this line!!)

; fix defstruct ppc-operand for new enforcement of :type
; 01/31/00 AKH altivec from Shannon Spires
; 03/01/96 gb    uuo_fpuXbinop
; 01/19/96 bill  Wrap up top-level initialization form in initialize-ppc-opcode-numbers
;                function, make it call fixup-vinsn-templates, and add comment warning to
;                call it if you change *ppc-opcodes*.
; 01/10/96 gb use halfwords a lot of the time to avoid consing bignums.
; 01/06/96 gb new uuo_fixnum_overflow
; 11/11/95 gb new UUOs; fix some instruction macros.
...
(defenum ()
  ...
  $uuo-errnum
  $uuo-small-errnum
  $va               ; The vA field in a vector instruction. SVS.
...
(eval-when (:compile-toplevel :load-toplevel :execute)
(defstruct ppc-operand
  (index 0 :type unsigned-byte)
  (width 0 :type (mod 33)) ; Was 32 - fails for mbe-aux whatever that is
  (offset 0 :type (mod 32))
  (insert-function nil :type (or null symbol function))
  (extract-function nil :type (or null symbol function))
  (flags 0 :type fixnum))
...
(defmethod make-load-form ((o ppc::ppc-operand) &optional env)
  (declare (ignore env))
...
(defparameter *ppc-operands*
  (vector
   ...
   (ppc-op $bat 5 16 insert-bat extract-bat $ppc-operand-fake)
   (ppc-op $bb 5 11 nil nil $ppc-operand-cr)
   (ppc-op $bba 5 11 insert-bba extract-bba $ppc-operand-fake)
   ...
   (ppc-op $obf 3 23 insert-bf extract-bf $ppc-operand-cr $ppc-operand-optional)
   ...
   (ppc-op $cr 5 16 insert-cr extract-cr $ppc-operand-cr $ppc-operand-optional)
   ...
   (ppc-op $fxm 8 12)
   (ppc-op $l 1 21 nil nil $ppc-operand-optional)
   ...
   (ppc-op $mb 5 6)
   (ppc-op $me 5 1 )
   (ppc-op $mbe 5 6 nil nil $ppc-operand-optional $ppc-operand-next)
   ...
   (ppc-op $rbs 5 11 insert-rbs extract-rbs $ppc-operand-fake)
...
(eval-when (:load-toplevel :execute)
  (dotimes (i (length *ppc-operands*))
    (unless (= i (ppc-operand-index (svref *ppc-operands* i)))
...
(defmacro uuo (xop) `(op 0 (dpb ,xop (byte 7 4) (logior (ash 1 3) fulltag-imm))))
...
(defmethod print-object ((p ppc-opcode) stream)
  (declare (ignore depth))
  (print-unreadable-object (p stream :type t) 
    (format stream "~a" (string (ppc-opcode-name p)))))

#-:ansi-make-load-form
(defmethod make-load-form ((p ppc-opcode))
  (make-load-form-saving-slots p))

#+:ansi-make-load-form
(defmethod make-load-form ((p ppc::ppc-opcode) &optional env)
  (declare (ignore env))
  (make-load-form-saving-slots p))
...
(defun max-operand-count (opnums)
  (let* ((max 0))
    ...
    (dolist (i opnums max)
      (unless 
        (logbitp $ppc-operand-fake (ppc-operand-flags (svref *ppc-operands* i)))
        (incf max)))))
...
(defun min-operand-count (opnums)
  (let* ((min 0))
    (declare (fixnum min))
    (dolist (i opnums min)
      (let* ((flags (ppc-operand-flags (svref *ppc-operands* i))))
        (declare (fixnum flags))
        (unless (or (logbitp $ppc-operand-fake flags)
                    (logbitp $ppc-operand-optional flags))
          (incf min))))))

(defmacro ppc-opcode (name opcode mask (&rest flags) &rest operands)
  `(make-ppc-opcode :name (string ',name)
      :opcode ,opcode
...
(defparameter *ppc-opcodes*
  (vector
   #.(ppc-opcode uuo_UNUSED0 (uuo 1) $uuorb-mask ($ppc) $rt $ra)
   #.(ppc-opcode uuo_unbound (uuo 2) $uuorb-mask ($ppc) $rt $ra)
   #.(ppc-opcode uuo_slot_unbound (uuo 3) $uuo-mask ($ppc) $rt $ra $rb)
   #.(ppc-opcode uuo_vector_bounds (uuo 4) $uuo-mask ($ppc) $ra $rb)
...
; other opcode's opcode numbers stay constant.
   #.(ppc-opcode uuo_fixnum_overflow (uuo 21) $uuorb-mask ($ppc) $rt $ra)
   #.(ppc-opcode uuo_fpuXbinop (uuo 22) $uuo-mask ($ppc) $frt $fra $frb)
   #.(ppc-opcode uuo_multiply_fixnums (uuo 23) $uuo-mask ($ppc) $rt $ra $rb)
   #.(ppc-opcode uuo_xalloc (uuo 24) $uuo-mask ($ppc) $rt $ra $rb)
...
(defun initialize-ppc-opcode-numbers ()
    ...
    (dotimes (i (length *ppc-opcodes*))
      (let* ((code (svref *ppc-opcodes* i))
             (opcode (ppc-opcode-opcode code))
             (mask (ppc-opcode-mask code)))
        (setf (gethash (string (ppc-opcode-name code))  *ppc-opcode-numbers*) i)
        (setf (ppc-opcode-op-high code) (ldb (byte 16 16) opcode)
              (ppc-opcode-op-low code) (ldb (byte 16 0) opcode)
              (ppc-opcode-mask-high code) (ldb (byte 16 16) mask)
              (ppc-opcode-mask-low code) (ldb (byte 16 0) mask))                                           
        (let* ((op (ppc-opcode-majorop code)))
    ...
    (when (fboundp 'ccl::fixup-vinsn-templates)   ; not defined yet at bootstrap time
      (funcall 'ccl::fixup-vinsn-templates)))
...
(defun insert-mb6 (instr val)
  (logior (logand val #x20) (dpb val (byte 5 6) instr)))

(defun extract-mb6 (instr)
  (logior (logand instr #x20) (ldb (byte 5 6) instr)))
...
(defun insert-sh6 (instr val)
  (logior (dpb val (byte 5 11) instr) (ash (logand val #x20) -4)))

(defun extract-sh6 (instr)
  (logior (ldb (byte 5 11) instr) (logand (ash instr 4) #x20)))
...
(defun insert-default (operand high low val)
  (let* ((width (ppc-operand-width operand))
         (offset (ppc-operand-offset operand))
...
(defun extract-default (operand instr)
  (let* ((width (ppc-operand-width operand))
	   (op (ldb (byte width (ppc-operand-offset operand)) instr)))
    (if (and (logbitp $ppc-operand-signed (ppc-operand-flags operand))
	        (logbitp (1- width) op))
	 (- op (ash 1 width))
       op)))
    ...
    (unless (= k -1)
      (dotimes (j (svref *ppc-opcode-counts* op))
        ...
        (let* ((code (svref *ppc-opcodes* (+ k j))))
          (if (= (logand (ppc-opcode-mask code) i)
                 (ppc-opcode-opcode code))
            (if (dolist (op (ppc-opcode-operands code) t)
                  (let* ((xfun (ppc-operand-extract-function op)))
                    (unless (or (null xfun)
                                (funcall xfun i))
                      (return nil))))
              (return code))))))))
...
(defun ppc-disasm-1 (i)
  (let* ((opcode (find-ppc-opcode i)))
    (if (null opcode)
      `(".long" ,i)
      (let* ((vals ()))
        (dolist (operand (ppc-opcode-operands opcode))
          (unless (logbitp $ppc-operand-fake (ppc-operand-flags operand))
            (let* ((extract-fn (ppc-operand-extract-function operand)))
              (push (if extract-fn
