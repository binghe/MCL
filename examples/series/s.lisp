...
;Copyright 1982,1983,1984,1985,1986,1987,1988,1989,1990, and 1991
...
;This file ("S.LISP") contains the 2/8/91 version of the Series macro
;package.  It implements efficient computation with series
;expressions in Common Lisp.  The functions in this file
;are documented in Appendices A and B of
;  Common cl: the Language, Second Edition, Guy L. Steele Jr, Digital press, 1990,
...
(defpackage "SERIES" (:use "COMMON-LISP"))
(in-package "SERIES")
...
(setf (get 'ccl:compiler-let 'scan-template) #'compiler-let-template)
...
;  ccl:compiler-let FLET LABELS MACROLET but must not macroexpand.
...
(cl:defun define-optimizable-series-fn (name lambda-list expr-list)
  ...
  (cl:let ((call (list* 'defun name lambda-list expr-list))
	...
	`(cl:defun ,name ,lambda-list
	   ...
	   (ccl:compiler-let ((*optimize-series-expressions* nil)) ,@ forms))))))

    ...
    `(eval-when (eval load compile)
       ...
       ,@(cond ((and (not trigger) (symbolp body-code))
		...
		`((cl:defun ,(if trigger body-fn name) ,arglist
		    ,@(if doc (list doc))
		    ,@(if dcl (list dcl))
		    (ccl:compiler-let ((*optimize-series-expressions* nil)) ,body-code)))))
       ,@(if discriminator
	     `((cl:defun ,desc-fn (call) ,discriminator)))
       (setf (get ',name 'returns-series) (function ,desc-fn))
       (cl:defun ,opt-fn ,opt-arglist
	 ,@(if dcl (list dcl))
	 (ccl:compiler-let ((*optimize-series-expressions* T)) ,opt-code))
    ...
    (cond (cached-value)
	  (T (setq cached-value
		   (top-starting-series-expr call
		      ...
		      `(ccl:compiler-let ((*optimize-series-expressions* nil)) ,call)))
...
(cl:defun scan-multi-out->scan-list-out (fn type init step test)
  (ccl:compiler-let ((*optimize-series-expressions* nil))
    (cl:let ((n (length (decode-type-arg type))))
