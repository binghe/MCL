...
;;; #$dragtracking... => #$kdragtracking..., use #_getregionbounds, #$zoom... => #$kzoom..., #$drag...=> #$kdrag...
;;; drag-add-hfs-flavor - use fsref insofar as possible
;;; 05/20/05 fix for non MCL drag source, fix some (not all) of alias resolution stuff
;;; ------ 5.1 final
;;; fix drag-create-item-bounds to release the region from Takehiko Abe
;;; --------- 5.1b3
;;; 07/07/04 fix drag-get-drop-location-as-path
;;; 06/30/04 fix with-timer fix
;;; --------- 5.1b2 
;;; 04/18/04 do  with-timer in two places
;;; -------- 5.1b1
;;; akh no mo hopenresfile - more to do re fsrefs
;;; --------- 5.0 final
;;; 10/26/02 %special-folder-path --> findfolder
;;; 10/09/02 akh no more aedesc.datahandle
;;; 12/09/01 another oddness in DragTrackingHandler... when Carbon
;;; 05/30/01 something odd re osx-p - see DragTrackingHandlerDispatch.p 
...
(defpascal DragTrackingHandlerDispatch.p (:word $theMessage :ptr $theWindow
  ...
  (let ((w (%wptr-to-drag-window $theWindow))
    ...
    (handler-case
      (when w        
        (if (not (eql $dragReference (reference *drag-session*)))            
          (progn
            ;; happens a lot (aka always) when osx - not otherwise at least just using IFT
            #+ignore
            (with-pstrs ((p (format nil "reset-drag0 in-ref ~x sess-ref ~x" 
                                    $dragreference
                                    (reference *drag-session*))))
              (#_debugstr p))
            (when t ;(not (mcl-drag-p *drag-session*))
              (%drag-reset-drag-session $dragReference))))
        (let ((v (drag-mouse-view w)))  ;; moved this AFTER dragreference is correct in *drag-session*
...
(defun %drag-new-drag-session ()
  ...
  (rlet (($dragRef :DragReference))
    ...
    (setf (reference *drag-session*) (%get-ptr $dragref) ; (%get-long $dragRef)) ;; huh
    ...
    (when result
      (let ((mouse-pos (drag-mouse-position nil)))
        (rlet ((dest-rect :rect
          ...
          (progn ;with-dereferenced-handles ((region-ptr (drag-region *drag-session*)))  ;; bad!
            (rlet ((rect :rect))
              (#_getregionbounds (drag-region *drag-session*) rect) 
              (drag-zoom-rectangle rect
                                   dest-rect
                                   :zoom-steps 24)))))
      result)))
  ...
  (if (eql (drag-state *drag-session*) :setup)
    (let ((result nil))
      (when (or (pathnamep path) (stringp path))
        (with-pstrs ((path-str (mac-namestring path)))  ;; needs fixing
          (rlet (($hfs :HFSFlavor)
                 ($fs :pointer)
                 ($info :FInfo))
    ...
    (let ((result nil))
      (when (or (pathnamep path) (stringp path))
        (rlet ((fsref :fsref)
               ...
               ($hfs :HFSFlavor)
               (fsspec :ptr))
          (%setf-macptr fsspec (%inc-ptr $hfs #.(get-field-offset :HFSFlavor.fileSpec)))          
          ;(%path-to-fsspec path fsspec :quiet)  ;; huh - why both?
          (make-fsref-from-path path fsref t)
          (oserr-check (#_FSGetCatalogInfo fsref #$kFSCatInfoGettableInfo  ;; need all for fsspec??
                        catinfo (%null-ptr) fsspec (%null-ptr)))           
          (setf (pref $hfs :HFSFlavor.fdFlags) (pref catinfo :FSCataloginfo.FinderInfo.finderflags)
                (pref $hfs :HFSFlavor.fileType) (pref catinfo :FSCataloginfo.FinderInfo.filetype)
                (pref $hfs :HFSFlavor.fileCreator) (pref catinfo :FSCataloginfo.FinderInfo.filecreator))
          (setf result (drag-add-item-flavor item-reference flavor
                                             $hfs #.(record-length :HFSFlavor)
                                             flavor-flag))))
      result)))
...
(defmethod drag-set-hfs-flavor ((item-reference integer) (flavor keyword) (path t))
  (if (dragging-now-p)
    (let ((result nil))
      (when (or (pathnamep path) (stringp path))
        (rlet ((fsspec :fsspec))
          (%path-to-fsspec path fsspec :none)          
          (setf result (drag-set-item-flavor item-reference flavor
                                             fsspec #.(record-length :FSSpec)))))
      result)))
  ...
  (if (eql (drag-state *drag-session*) :setup)
    (let ((finder-flag-value 0))
      ...
      (if (position :has-been-inited finder-flags)
        (incf finder-flag-value #$kHasBeenInited))                ; #$kHasBeenInited
      (if (position :is-stationery finder-flags)
        (incf finder-flag-value #$kIsStationery))                ; #$kIsStationery
      (if (position :name-locked finder-flags)
        (incf finder-flag-value #$kNameLocked))                ; #$kNameLocked
      (if (position :has-bundle finder-flags)
        (incf finder-flag-value #$kHasBundle))                ; #$kHasBundle
      (if (position :is-invisible finder-flags)
        (incf finder-flag-value #$kIsInvisible))                ; #$kIsInvisible
      (if (position :is-alias finder-flags)
        (incf finder-flag-value #$kIsAlias))                ; #$kIsAlias
...
(defmethod drag-get-hfs-flavor ((item-ref-or-ptr macptr) (hfs-flavor t)
                                &optional (resolve-aliases-p nil))
  (declare (ignore hfs-flavor))
  (let ((path (%path-from-fsspec (pref item-ref-or-ptr :HFSFlavor.fileSpec))))
    (when resolve-aliases-p
      (let ((*alias-resolution-policy* :quiet))
        (rlet ((fsref :fsref))
          (when (path-to-fsref path fsref)
            (setq path (%path-from-fsref fsref))))))
    (values path
            (pref item-ref-or-ptr :HFSFlavor.fileCreator)
            (pref item-ref-or-ptr :HFSFlavor.fileType)
            (pref item-ref-or-ptr :HFSFlavor.fdFlags))))
        ...
        (when hfs-flavor
          (with-drag-flavor (item-reference hfs-flavor hfs-ptr ptr-size)
            ...
            (if resolve-aliases-p
              ...
              (setf path (%path-from-fsspec hfs-ptr)))))
...
(defun drag-get-drop-location-as-path ()
  (let ((drop-location (drag-get-drop-location-descriptor)))
    ;; assuming descriptorType is :|alis| or :|fss |
    (if drop-location
      (ecase (rref drop-location :aedesc.descriptorType)
        (#.#$typealias (drag-get-drop-location-from-alias drop-location)) ;(%resolve-alias-handle-without-dialog (rref drop-location :AEDesc.dataHandle)))
        (#.#$typeFSS (drag-get-drop-location-from-fsspec drop-location))))))

(defun drag-get-drop-location-from-alias (drop-location)
  (let* ((data-size (#_aegetdescdatasize drop-location)))
    (rlet ((alias-handle-ptr :aliashandle))
      (%stack-block ((data-ptr data-size))
        (errchk (#_aegetdescdata drop-location data-ptr data-size))  ;; assume this copies to data-ptr
        ;(%resolve-alias-handle-without-dialog data-ptr))))))
        (errchk (#_ptrtohand data-ptr alias-handle-ptr data-size))   ;; does this copy? - assume so - lotta copies         
        (%resolve-alias-handle-without-dialog (%get-ptr alias-handle-ptr))))))
    

;; ??? wrong
(defun drag-get-drop-location-from-fsspec (drop-location)
  (let* ((data-size (#_aegetdescdatasize drop-location)))
    (%stack-block ((data-ptr data-size))
      (errchk (#_aegetdescdata drop-location data-ptr data-size))  ;; assume this copies to data-ptr
      (let ((path (%path-from-fsspec data-ptr)))
        (%resolve-alias-file-without-dialog path)))))
...
(defun drag-set-drop-location-from-path (path)
  (when (dragging-now-p)
    (let ((path-name (mac-namestring path))
          (result nil))
      (with-cstrs ((path-string path-name))  ;; needs fixing
        ...
        (unwind-protect
          (progn
            (let ((*alias-resolution-policy* :none)) ;; ??            
              ...
              (oserr-check (#_FSNewAliasMinimal fsref alias)))
            (if (drag-set-drop-location-from-alias (%get-ptr alias))
              (setf result path)))
          (unless (%null-ptr-p (%get-ptr alias))
            (#_DisposeHandle (%get-ptr alias)))))
        ...
        (let ((drop-location (drag-get-drop-location-as-path)))
          (when drop-location 
            (let ((resolved-drop (or (%resolve-alias-file-without-dialog drop-location)
              ...
              (if (equalp resolved-drop (findfolder #$kuserdomain #$kTrashFolderType) ; much better for OSX
                          )
...
(defmethod drag-create-item-bounds ((view simple-view) (item-reference integer)
  ...
  (when (dragging-now-p)
    (with-focused-view view
      (let ((global-point (local-to-global view (make-point 0)))
            (temp-region (#_newrgn)))
        (unwind-protect
          (progn
            (#_copyrgn item-region temp-region)
            (when outline-p
...
;; Should use findfolder instead of this routine
#+unnecessary
...
;; a directory. - %path-from-fsspec fixed now
#|
...
(defun %resolve-alias-handle-without-dialog (alias-handle)
  (let* ((resulting-path nil)
    ...
    (rlet ((max-to-find :signed-integer)
      ...
      (setf err (#_MatchAliasNoUI (%null-ptr) search-params alias-handle max-to-find
...
(defun %resolve-alias-handle-without-dialog (alias-handle)
  (rlet ((fsref :fsref)
         (was-changed :boolean))
    (let ((err (#_FSResolveAliasWithMountFlags 
                (%null-ptr) 
                alias-handle 
                fsref 
                was-changed
      ...
      (if (eql err #$noerr)
        (let ((path (%path-from-fsref fsref)))
          ;(%resolve-alias-file-without-dialog path)
          path)
        (progn (ed-beep)
               nil)))))
      ...
      (if (eql (#_FSpGetFInfo path-or-fsspec file-info) #$noErr)
        (if (logbitp 15 (pref file-info :FInfo.fdFlags))
          (let ((file-path (%path-from-filespec path-or-fsspec))
            ...
            (when (neq res-refnum -1) ;(#_ResError) #$noErr)
...
(defmethod %resolve-alias-file-without-dialog ((path-or-fsspec macptr))
  (let ((pathname (%path-from-fsspec path-or-fsspec)))
    (%resolve-alias-file-without-dialog pathname)
    ))
