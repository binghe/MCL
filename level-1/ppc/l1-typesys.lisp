...
;;  $Log: l1-typesys.lisp,v $
;;  Revision 1.10  2006/03/21 20:22:40  alice
;;  ;; short-float = single-float
;; ---- 5.2b1
;;
;;  Revision 1.9  2003/12/29 04:18:27  gtbyers
;;  Try real hard to compute type predicate for effective slot definitions.
;;
;;  Revision 1.8  2003/12/08 08:31:36  gtbyers
...
;; Change History
;;
;; type-intersection and type-union check that args are ctypes
;; add intersection-ctype to %%typep and ctypep
;; define-type-method (array :unparse)
;;     add base-char = base-character, put back elt type character -> string
;; say signal-program-error vs error in a few places
;; define-type-method (class :simple-subtypep) check for forward-referenced-superclass-p
;; ------- 5.2b6
;; fix nasty bug in ctypep which caused (typep :bar '(and keyword (member :foo :bar))) to fail
;; ---- 5.2b5
;; short-float = single-float
;; ---- 5.2b1
;; akh fix parens
;; akh deftype simple-short-float-vector
;; ------ 5.0b5
;; akh deftype signed-byte
;; --------- 4.3.1b2
;; akh fix some misspelled decls (doesn't really matter - just prevents warnings)
;; -------- 4.3f1c1
;; 05/17/99 akh add info-type-builtin for atom
;; -------- 4.3b2
;; 04/18/99 akh deftype for signed and unsigned byte special case 8 to avoid consing in type-expand
;; 03/19/99 akh %deftype removes from type-cache, add cons compound type (boring)
;; --------------- 4.3b1 
;; 10/05/98 akh %%typep - don't cons array-dimensions for 1d array
...
;; hopefully these will be exported someday
#+ignore
(eval-when (:compile-toplevel :execute)

;; these are also in compiler;ppc;ppc-arch.lisp in ppc package
    ...
    `(progn
       (let* ((fn #'(lambda ,lambda-list ,@body)))
         ,@(mapcar #'(lambda (method)
                       `(setf (%svref (type-class-or-lose ',class)
...
(defun ctype-p (x)
  (and (eql (ppc-typecode x) ppc::subtag-istruct)
       (memq (%svref x 0) 
             '#.(cons 'ctype 
                      (cons 'unknown-ctype                            
...
(defun args-type-op (type1 type2 operation nreq default-type)   ;; oops - has add'l arg
...
(defun type-union (&rest input-types)
  (dolist (type input-types)
    (when (not (ctype-p type))
      (signal-type-error type 'ctype)))
...
|#
 ...
 ;; openMCL version
  (defun values-specifier-type (spec)
    (if (typep spec 'class)
      ...
      (if locked
        (or (values-specifier-type-internal spec)
            (make-unknown-ctype :specifier spec))
        (or (info-type-builtin spec)  ;; added this 29 jan 07 - maybe not needed
            (unwind-protect
              (progn
                (setq locked t)
                (if (or (symbolp spec)
                  ...
                  (let* ((idx (hash-type-specifier spec)))
                    ...
                    (if (equal (svref type-cache-specs idx) spec)
                      ...
                      (let* ((ctype (values-specifier-type-internal spec)))
                        (if ctype
		          (progn
			    (when (and (cacheable-ctype-p ctype))   ;; <<<
			      (setf (svref type-cache-specs idx) (copy-tree spec)       ; in case it was stack-consed
				    (svref type-cache-ctypes idx) ctype))
			    ctype)
                          (make-unknown-ctype :specifier spec)))))
  ...
  (defun remove-from-type-cache (spec)
    (when (or (symbolp spec)
                    (and (consp spec) (symbolp (car spec))))
      (let ((idx (hash-type-specifier spec)))
        (when (equal (svref type-cache-specs idx) spec)
          (setf (svref type-cache-specs idx) 0)
          (setf (svref type-cache-ctypes idx) 0)))))
  ...
  (let* ((not-type (specifier-type typespec))
	...
	(cond
	 ...
	 (if (some #'floatp members)
	   (let (floats)
	     (dolist (pair '((0.0s0 . -0.0s0) (0.0d0 . -0.0d0)))
        ...
        :dimensions (cond ((eq dims1 '*) dims2)
                          ...
                          (t
                           (mapcar #'(lambda (x y) (if (eq x '*) y x))
                                   dims1 dims2)))
        :complexp (if (or (eq complexp1 '*)(eq complexp1 :maybe)) complexp2 complexp1)
        :element-type (cond
...
(defparameter specialized-array-element-types
  '(nil bit (unsigned-byte 8) (signed-byte 8) (unsigned-byte 16) 
    ...
    base-character character single-float double-float))
...
;;;; cons types - kinda boring -- see also an additional clause in %%typep, and in ctypep
;;; lots of this stuff belongs elsewhere - e.g. lispequ, l1-clos
...
(defun cons-ctype-p (x) (istruct-typep x 'cons-ctype))
(setf (type-predicate 'cons-ctype) 'cons-ctype-p)


;; should be in lispequ, I guess - it is
...
(new-type-class 'cons)  ;; should be in l1-clos
...
#|
(deftype barf () '(cons integer cons))
(deftype zz () '(cons double-float cons))
(deftype yy () '(cons fixnum cons))
(deftype qq () '(cons fixnum))
(deftype barf2 () '(cons integer fixnum))
(typep 4 'barf)
(typep '(1 2) 'yy)
(typep '(1 2 3) 'barf2)
(subtypep 'barf 'cons)  
...
(defun make-member-ctype (&key members)
  (let* ((singlep (subsetp '(-0.0s0 0.0s0) members))
    ...
    (if union-types
      (progn
	(if singlep
	  (setq members (set-difference '(-0.0s0 0.0s0) members)))
	(if doublep
...
;; add the rest of intersection someday
...
(defun ctypep (obj type)
  ...
  (etypecase type
    #|
    (numeric-ctype  (values (%typep obj type) t))
    (named-ctype  (values (%typep obj type) t))
    (member-ctype  (values (%typep obj type) t))
    (array-ctype  (values (%typep obj type) t))
    (cons-ctype  (values (%typep obj type) t)) |#
    ((or numeric-ctype named-ctype member-ctype array-ctype cons-ctype)
     (values (%typep obj type) t))
    (class-ctype
     (if (and (uvectorp obj)
              (or (eq (%vect-subtype obj) #.ppc::subtag-instance )))
      ...
      (array-ctype
       (and (arrayp object)
            ...
            (or (eq (array-ctype-dimensions type) '*)
                (let ((ctype-dims (array-ctype-dimensions type)))
                  (if (and (eq (array-rank object) 1)(eq (length ctype-dims) 1))
                    (if (or (eq (car ctype-dims) '*)
                            (eql (car ctype-dims) (length object)))
                      t
                      nil)
                    (do ((want ctype-dims (cdr want))
...
(defvar *ctype-of-single-float-0* (ctype-of 0.0s0))
...
(deftype unsigned-byte (&optional s)
  (cond ((eq s '*) '(integer 0))
	((and (integerp s) (> s 0))
	   ...
	   `(integer 0 ,(1- (ash 1 s)))))
	(t
	 (signal-program-error "Bad size specified for UNSIGNED-BYTE type specifier: ~S." s))))
...
(flet ((set-builtin-class-type-translation (thing)
  ...
  (mapc #'set-builtin-class-type-translation
        '(
          ...
          simple-array
          ;;  Vectors
          vector string extended-string base-string bit-vector
          ...
          simple-1d-array  simple-string simple-base-string simple-extended-string
          simple-bit-vector
...
;;; DEFTYPE tries to globally define the type
;;; at compile-time as well as load- and execute time, so hide
;;; the definitions of these "built-in" types. 
...
(queue-fixup
  (clear-type-cache)  ;; oh my goodness
  ;; I dunno why we need this stuff
  (setf (info-type-builtin 'sequence) (specifier-type '(or cons (member nil) vector)))  
  (setf (info-type-builtin 'list) (specifier-type '(or cons (member nil))))
  (setf (info-type-builtin 'atom)(specifier-type '(not cons)))
  ;; why needed ???
   (setf (info-type-builtin 'ratio) (specifier-type '(and rational (not integer)))))

;(setq *use-hairy-cons-type* t)  ;; not yet - need to compile the compiler having the right defs in lispequ.lisp loaded
