...
;;  4 7/18/96  akh  the fixnum in linevec-ref
...
;
; change comment re: bf.owner - its pathname now
; ----- 5.2b5
; add 2 more chars to wsp&cr and symbol-specials
;; ----- 5.2b1
; 10/16/05 unicode non breaking space in wsp&cr etal
; add bf.owner slot to buffer
; ------ 5.1 final
...
(makunbound 'wsp)
(defconstant wsp #.(let ((str (make-string 5  :element-type 'base-character)))
                      (set-schar str 0 #\Space)
                      (set-schar str 1 (code-char 9)) ;; #\tab
                      (set-schar str 2 (code-char 12)) ;; #\page - form feed
                      (set-schar str 3 (code-char 0)) ;; #\null
                      ;;(set-schar str 4 #\^J)  NO NO that's linefeed
                      (set-schar str 4 (code-char #xa0))  ;; non breaking space
                      str))
...
(defconstant symbol-specials #.(let ((str (make-string 18 :element-type 'extended-character)))
                                 (set-schar str 0 #\Space)
                                 (set-schar str 1 #\()
                                 (set-schar str 2 #\))
                                 (set-schar str 3 (code-char 13))  ;; #\return
                                 (set-schar str 4 #\')
                                 (set-schar str 5 #\`)
                                 (set-schar str 6 #\,)
                                 (set-schar str 7 #\")
                                 ...
                                 (set-schar str 9 #\\)
                                 (set-schar str 10 #\|)
                                 (set-schar str 11 (code-char 9)) ;; #\tab
                                 (set-schar str 12 (code-char 12)) ;; #\page
                                 (set-schar str 13 (code-char 0)) ;; #\null
                                 (set-schar str 14 (code-char 10)) ;; #\linefeed
                                 (set-schar str 15 (code-char #xa0)) ;; non breaking space
                                 (set-schar str 16 (code-char #x2028))
                                 (set-schar str 17 (code-char #x2029))
                                 str))
...
(def-accessors (buffer) %svref
  ...
  bf.undo-redo
  bf.fred-history
  bf.owner                       ; pathname of source file if any
)
...
(defmacro linevec-ref (linevec index)
  `(the fixnum (ccl::%svref ,linevec ,index)))
