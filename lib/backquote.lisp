...
;03/14/97 bill change bq-process in the commented out Steele code to handle vectors.
;              That code should be safe to use now.
; -----------  4.0
;07/21/91 gb  DECLAIM.
;12/28/90 bill in backquote-aux: CONS -> LIST*
;11/01/90 gb fix some typos in gls code, still commented out.
;2/21/90  gz Put in the backquote code from steele, haven't enabled
;            it yet (needs to be made to handle vectors).
;12/27/89 gz Removed (put 'version), provide at end.
;9/29/88 gb  call set-macro-character on functions, not symbols.
;8/25/88 gz  Simple vectors are now self-evaluating...
;8/03/87 gz  distinquish self-induced quoting from random quoted forms.
;7/30/87 gz  Make Jim Meehan happy.
;7/12/87  gb Provide thyself.
;6/16/87 gz  if *backquote-expand* non-nil, expand at read-time
;6/13/87 gb  proclaim specials.
;5/4/87  gz  modified to handle nested commas!
;4/16/87 gz  modified to handle nested backquotes.
;4/2/87  gz  added 'version put.
;02/17/87 gz Use atoms for markers, because conses don't always get dumped uniquely.
;02/10/87 gb got rid of this LENGTH.  Are we real yet?
