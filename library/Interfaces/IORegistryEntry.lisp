(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IORegistryEntry.h"
...
(def-mactype :gIONameKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOLocationKey (find-mactype '(:pointer :OSSymbol)))
...
(def-mactype :IORegistryEntryApplierFunction (find-mactype ':pointer)); (IORegistryEntry * entry , void * context)
...
; 
#|
 confused about CLASS IORegistryEntry #\: public OSObject #\{ friend class IORegistryIterator #\; OSDeclareDefaultStructors #\( IORegistryEntry #\) protected #\:
...
;     Reserved for future use.  (Internal use only)  
 ExpansionData * reserved #\; private #\: OSDictionary * fRegistryTable #\; OSDictionary * fPropertyTable #\; public #\:;  methods available in Mac OS X 10.1 or later 
...
;     @result A pointer to the plane object, or zero if no such plane exists. The returned plane should not be released. 
 static const IORegistryPlane * getPlane #\( const char * name #\) #\;;  Registry Entry allocation & init 
; ! @function init
...
;     @result True on success, false otherwise. 
 virtual bool serializeProperties #\( OSSerialize * serialize #\) const #\;;  Unsynchronized(!) property table access 
...
;  inline 
 OSDictionary * getPropertyTable #\( void #\) const #\;;  { return(fPropertyTable); } 
...
;     @result An IOReturn code to be returned to the caller. 
 virtual IOReturn setProperties #\( OSObject * properties #\) #\;;  Topology 
...
;     @result Returns an iterator over the parents of the registry entry, or zero if there is a resource failure. The iterator must be released when the iteration is finished. All objects returned by the iteration are retained while the iterator is valid, though they may no longer be attached during the iteration. 
 virtual OSIterator * getParentIterator #\( const IORegistryPlane * plane #\) const #\; virtual void applyToParents #\( IORegistryEntryApplierFunction applier #\, void * context #\, const IORegistryPlane * plane #\) const #\;
...
;     @result Returns an iterator over the children of the entry, or zero if there is a resource failure. The iterator must be released when the iteration is finished. All objects returned by the iteration are retained while the iterator is valid, though they may no longer be attached during the iteration. 
 virtual OSIterator * getChildIterator #\( const IORegistryPlane * plane #\) const #\; virtual void applyToChildren #\( IORegistryEntryApplierFunction applier #\, void * context #\, const IORegistryPlane * plane #\) const #\;
...
;     @result The maximum number of entries between the entry and the root. Zero is returned if the entry is not attached in the plane. 
 virtual unsigned int getDepth #\( const IORegistryPlane * plane #\) const #\;;  Attach / detach 
...
;     @param plane The plane object, or zero to set the global location string. 
 virtual void setLocation #\( const OSSymbol * location #\, const IORegistryPlane * plane = 0 #\) #\; virtual void setLocation #\( const char * location #\, const IORegistryPlane * plane = 0 #\) #\;
; ! @function getPath
...
;  * * * * * * * * * * * internals * * * * * * * * * * * 
;  * * * * * * * * * * * * * * * * * * * * * * * * * * * 
 public #\: static IORegistryEntry * initialize #\( void #\) #\; static const IORegistryPlane * makePlane #\( const char * name #\) #\;;  don't even think about using this
 virtual bool init #\( IORegistryEntry * from #\, const IORegistryPlane * inPlane #\) #\; private #\: inline bool arrayMember #\( OSArray * set #\, const IORegistryEntry * member #\, unsigned int * index = 0 #\) const #\; bool makeLink #\( IORegistryEntry * to #\, unsigned int relation #\, const IORegistryPlane * plane #\) const #\; void breakLink #\( IORegistryEntry * to #\, unsigned int relation #\, const IORegistryPlane * plane #\) const #\; virtual OSArray * getParentSetReference #\( const IORegistryPlane * plane #\) const #\; virtual OSArray * getChildSetReference #\( const IORegistryPlane * plane #\) const #\; virtual IORegistryEntry * getChildFromComponent #\( const char ** path #\, const IORegistryPlane * plane #\) #\; virtual const OSSymbol * hasAlias #\( const IORegistryPlane * plane #\, char * opath = 0 #\, int * length = 0 #\) const #\; virtual const char * matchPathLocation #\( const char * cmp #\, const IORegistryPlane * plane #\) #\;
|#
;  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
...
#|
 confused about CLASS IORegistryIterator #\: public OSIterator #\{ OSDeclareAbstractStructors #\( IORegistryIterator #\) private #\: struct IORegCursor #\{ IORegCursor * next #\; IORegistryEntry * current #\; OSIterator * iter #\; #\} #\; IORegCursor start #\; IORegCursor * where #\; IORegistryEntry * root #\; OSOrderedSet * done #\; const IORegistryPlane * plane #\; IOOptionBits options #\; virtual void free #\( void #\) #\; public #\:
...
(provide-interface "IORegistryEntry")