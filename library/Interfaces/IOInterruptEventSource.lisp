(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOInterruptEventSource.h"
...
; 
#|
 confused about CLASS IOInterruptEventSource #\: public IOEventSource #\{ OSDeclareDefaultStructors #\( IOInterruptEventSource #\) public #\:
; ! @typedef Action
...
; #define IOInterruptEventAction IOInterruptEventSource::Action
 protected #\:                                  ; ! @var provider IOService that provides interrupts for delivery. 
 IOService * provider #\;                       ; ! @var intIndex 
 int intIndex #\;
; ! @var producerCount
...
;     Current count of produced interrupts that the owner has been informed of. 
 unsigned int consumerCount #\;                 ; ! @var autoDisable Do we need to automatically disable the interrupt source when we take an interrupt, i.e. we are level triggered. 
 bool autoDisable #\;                           ; ! @var explicitDisable Has the user expicitly disabled this event source, if so then do not overide their request when returning from the callout 
...
;     @result Return true if this function needs to be called again before all its outstanding events have been processed. 
 virtual bool checkForWork #\( #\) #\; public #\:
...
;     @param nub Where did the interrupt originate from
;     @param ind What is this interrupts index within 'nub'. 
 virtual void disableInterruptOccurred #\( void * #\, IOService * nub #\, int ind #\) #\; private #\: OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 0 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 1 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 2 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 3 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 4 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 5 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 6 #\) #\; OSMetaClassDeclareReservedUnused #\( IOInterruptEventSource #\, 7 #\) #\;
|#

; #endif /* !_IOKIT_IOINTERRUPTEVENTSOURCE_H */


(provide-interface "IOInterruptEventSource")