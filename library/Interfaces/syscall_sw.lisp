(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:syscall_sw.h"
...
(require-interface "architecture/i386/asm_help")
; #define MACHCALLSEL     $0x07
; #define RPCCALLSEL 	$0x0f
; #define kernel_trap(trap_name,trap_number,number_args) LEAF(_##trap_name,0) ;	movl	$##trap_number,%eax   ;        lcall   MACHCALLSEL, $0 ;END(_##trap_name)
; #define rpc_trap(trap_name,trap_number,number_args) LEAF(_##trap_name,0) ;        movl    $##trap_number,%eax;         lcall   RPCCALLSEL, $0 ;END(_##trap_name)
; #define rpc_return_trap(trap_name,trap_number,number_args) LEAF(_##trap_name,0) ;	movl    %eax, %ecx;         movl    $##trap_number,%eax;         lcall   RPCCALLSEL, $0 ;END(_##trap_name)

; #endif	/* _MACH_I386_SYSCALL_SW_H_ */


(provide-interface "syscall_sw")