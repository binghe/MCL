(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOMemoryDescriptor.h"
; at Sunday July 2,2006 7:25:42 pm.
; 
...
(defconstant $kIODirectionNone 0)               ;                     same as VM_PROT_NONE

(defconstant $kIODirectionIn 1)                 ;  User land 'read',  same as VM_PROT_READ

(defconstant $kIODirectionOut 2)                ;  User land 'write', same as VM_PROT_WRITE

(defconstant $kIODirectionOutIn 3)
(defconstant $kIODirectionInOut 3)
; 
;  * IOOptionBits used in the second withRanges variant
;  

(defconstant $kIOMemoryDirectionMask 7)
(defconstant $kIOMemoryAutoPrepare 8)           ;  Shared with Buffer MD

(defconstant $kIOMemoryTypeVirtual 16)
(defconstant $kIOMemoryTypePhysical 32)
(defconstant $kIOMemoryTypeUPL 48)
(defconstant $kIOMemoryTypeMask #xF0)
(defconstant $kIOMemoryAsReference #x100)
(defconstant $kIOMemoryBufferPageable #x400)
(defconstant $kIOMemoryDontMap #x800)
(defconstant $kIOMemoryPersistent #x10000)
...
#|
 confused about CLASS IOMemoryDescriptor #\: public OSObject #\{ friend class _IOMemoryMap #\; friend class IOSubMemoryDescriptor #\; OSDeclareDefaultStructors #\( IOMemoryDescriptor #\) #\; protected #\:
...
;     Reserved for future use.  (Internal use only)  
 ExpansionData * reserved #\; protected #\: OSSet * _mappings #\; IOOptionBits _flags #\; void * _memEntry #\; IODirection _direction #\;;  DEPRECATED: use _flags instead. direction of transfer 
 IOByteCount _length #\;                        ;  length of all ranges 
 IOOptionBits _tag #\; public #\: virtual IOPhysicalAddress getSourceSegment #\( IOByteCount offset #\, IOByteCount * length #\) #\; OSMetaClassDeclareReservedUsed #\( IOMemoryDescriptor #\, 0 #\) #\;
...
;  inline 
 IOPhysicalAddress getPhysicalAddress #\( #\) #\;;  { return( getPhysicalSegment( 0, 0 )); } 
;  DEPRECATED 
;  USE INSTEAD: map(), readBytes(), writeBytes() 
;  DEPRECATED 
 virtual void * getVirtualSegment #\( IOByteCount offset #\,;  DEPRECATED 
 IOByteCount * length #\) = 0 #\;               ;  DEPRECATED 
;  USE INSTEAD: map(), readBytes(), writeBytes() 
; ! @function prepare
...
#|
 confused about CLASS IOMemoryMap #\: public OSObject #\{ OSDeclareAbstractStructors #\( IOMemoryMap #\) public #\:
...
;     @result An IOReturn code. 
 virtual IOReturn unmap #\( #\) = 0 #\; virtual void taskDied #\( #\) = 0 #\;
|#
;  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
...
(defconstant $kIOMemoryPreparedReadOnly #x8000)
#|
 confused about CLASS IOGeneralMemoryDescriptor #\: public IOMemoryDescriptor #\{ OSDeclareDefaultStructors #\( IOGeneralMemoryDescriptor #\) #\; protected #\: union #\{ IOVirtualRange * v #\; IOPhysicalRange * p #\; #\} _ranges #\;;  list of address ranges 
 ...
 union #\{ IOVirtualRange v #\; IOPhysicalRange p #\; #\} _singleRange #\;;  storage space for a single range 
 unsigned _wireCount #\;                        ;  number of outstanding wires 
;  DEPRECATED 
 vm_address_t _cachedVirtualAddress #\;         ;  a cached virtual-to-physical 
;  DEPRECATED 
 IOPhysicalAddress _cachedPhysicalAddress #\; bool _initialized #\;;  has superclass been initialized? 
 virtual void free #\( #\) #\; private #\:      ;  Internal API may be made virtual at some time in the future.
 IOReturn wireVirtual #\( IODirection forDirection #\) #\;;  DEPRECATED 
...
;      
 ...
 IOReturn redirect #\( task_t safeTask #\, bool redirect #\) #\; virtual bool serialize #\( OSSerialize * s #\) const #\; protected #\: virtual IOMemoryMap * makeMapping #\( IOMemoryDescriptor * owner #\, task_t intoTask #\, IOVirtualAddress atAddress #\, IOOptionBits options #\, IOByteCount offset #\, IOByteCount length #\) #\;
|#
;  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
...
(provide-interface "IOMemoryDescriptor")