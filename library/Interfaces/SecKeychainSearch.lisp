(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:SecKeychainSearch.h"
...
(require-interface "Security/SecKeychainItem")
...
(deftrap-inline "_SecKeychainSearchGetTypeID" 
   (
   )
   :UInt32
() )
...
(deftrap-inline "_SecKeychainSearchCreateFromAttributes" 
   ((keychainOrArray (:pointer :void))
    (itemClass :FourCharCode)
    (attrList (:pointer :SecKeychainAttributeList))
    (searchRef (:pointer :SecKeychainSearchRef))
   )
   :SInt32
() )
; !
...
(deftrap-inline "_SecKeychainSearchCopyNext" 
   ((searchRef (:pointer :OpaqueSecKeychainSearchRef))
...
(provide-interface "SecKeychainSearch")