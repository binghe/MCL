(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOFireWireAVCLib.h"
; at Sunday July 2,2006 7:28:55 pm.
; 
...
; #define kIOFireWireAVCLibUnitTypeID CFUUIDGetConstantUUIDWithBytes(NULL,		0x6A, 0xAF, 0x2E, 0xF7, 0xD4, 0x76, 0x11, 0xD5, 0xB5, 0x7C, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  Unit Factory UUID
;  3F4057BC-D479-11D5-9F05-0003934B81A0 
; #define kIOFireWireAVCLibUnitFactoryID CFUUIDGetConstantUUIDWithBytes(NULL, 	0x3F, 0x40, 0x57, 0xBC, 0xD4, 0x79, 0x11, 0xD5, 0x9F, 0x05, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  IOFireWireAVCUnitInterface UUID
;  FC65C030-D498-11D5-878D-0003934B81A0 
; #define kIOFireWireAVCLibUnitInterfaceID CFUUIDGetConstantUUIDWithBytes(NULL, 	0xFC, 0x65, 0xC0, 0x30, 0xD4, 0x98, 0x11, 0xD5, 0x87, 0x8D, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  Protocol type UUID
;  B54BC8F8-D53B-11D5-A1A1-0003934B81A0 
; #define kIOFireWireAVCLibProtocolTypeID CFUUIDGetConstantUUIDWithBytes(NULL,		0xB5, 0x4B, 0xC8, 0xF8, 0xD5, 0x3B, 0x11, 0xD5, 0xA1, 0xA1, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  Protocol Factory UUID
;  8E9AD5AC-D55E-11D5-B7D2-0003934B81A0 
; #define kIOFireWireAVCLibProtocolFactoryID CFUUIDGetConstantUUIDWithBytes(NULL,		0x8E, 0x9A, 0xD5, 0xAC, 0xD5, 0x5E, 0x11, 0xD5, 0xB7, 0xD2, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  IOFireWireAVCProtocolInterface UUID
;  CC85D421-D55E-11D5-8A10-0003934B81A0 
; #define kIOFireWireAVCLibProtocolInterfaceID CFUUIDGetConstantUUIDWithBytes(NULL,		0xCC, 0x85, 0xD4, 0x21, 0xD5, 0x5E, 0x11, 0xD5, 0x8A, 0x10, 0x00, 0x03, 0x93, 0x4B, 0x81, 0xA0)
;  IOFireWireAVCLibConsumerInterfaceID 
;  7FB7A454-226F-11D6-B889-000A277E7234 
; #define kIOFireWireAVCLibConsumerInterfaceID CFUUIDGetConstantUUIDWithBytes(NULL,		0x7F, 0xB7, 0xA4, 0x54, 0x22, 0x6F, 0x11, 0xD6, 0xB8, 0x89, 0x00, 0x0A, 0x27, 0x7E, 0x72, 0x34)

(def-mactype :IOFWAVCMessageCallback (find-mactype ':pointer)); (void * refCon , UInt32 type , void * arg)
; ! @typedef IOFWAVCRequestCallback
; 	@abstract This Callback has been deprecated. Use installAVCCommandHandler instead.
; 

(def-mactype :IOFWAVCRequestCallback (find-mactype ':pointer)); (void * refCon , UInt32 generation , UInt16 srcNodeID , const UInt8 * command , UInt32 cmdLen , UInt8 * response , UInt32 * responseLen)
; !
...
(def-mactype :IOFWAVCPCRCallback (find-mactype ':pointer)); (void * refcon , UInt32 generation , UInt16 nodeID , UInt32 plug , UInt32 oldVal , UInt32 newVal)
; !
...
(def-mactype :IOFWAVCCommandHandlerCallback (find-mactype ':pointer)); (void * refCon , UInt32 generation , UInt16 srcNodeID , IOFWSpeed speed , const UInt8 * command , UInt32 cmdLen)
; !
...
(def-mactype :IOFWAVCSubunitPlugHandlerCallback (find-mactype ':pointer)); (void * refCon , UInt32 subunitTypeAndID , IOFWAVCPlugTypes plugType , UInt32 plugNum , IOFWAVCSubunitPlugMessages plugMessage , UInt32 messageParams)

(def-mactype :IOFireWireAVCLibProtocolInterface (find-mactype ':_IOFireWireAVCLibProtocolInterface))
; !
...
; 
(defrecord IOFireWireAVCLibUnitInterface #|
   (NIL :iunknown_c_guts)|#
   (version :UInt16)
   (revision :UInt16)
                                                ; !
...
;     
   (open (:pointer :callback))                  ;(IOReturn (void * self))
                                                ; !
...
;         @result Returns kIOReturnSuccess on success.
;     
   (openWithSessionRef (:pointer :callback))    ;(IOReturn (void * self , IOFireWireSessionRef sessionRef))
                                                ; !
; 		@function openWithSessionRef
; 		@abstract Opens a connection to a device that is not already open.
; 		@discussion Sometimes it is desirable to open multiple user clients on a device.  In the case 
...
;     
   (getSessionRef (:pointer :callback))         ;(IOFireWireSessionRef (void * self))
                                                ; !
; 		@function close
...
;         @param self Pointer to IOFireWireAVCLibUnitInterface.
;     
   (close (:pointer :callback))                 ;(void (void * self))
                                                ; !
...
;         @result Returns kIOReturnSuccess on success.
;     
   (addCallbackDispatcherToRunLoop (:pointer :callback));(IOReturn (void * self , CFRunLoopRef cfRunLoopRef))
                                                ; !
...
;         @param self Pointer to IOFireWireAVCLibUnitInterface.
;     
   (removeCallbackDispatcherFromRunLoop (:pointer :callback));(void (void * self))
                                                ; !
...
;     
   (setMessageCallback (:pointer :callback))    ;(void (void * self , void * refCon , IOFWAVCMessageCallback callback))
                                                ; !
; 		@function AVCCommand
...
;     
   (AVCCommand (:pointer :callback))            ;(IOReturn (void * self , const UInt8 * command , UInt32 cmdLen , UInt8 * response , UInt32 * responseLen))
                                                ; !
...
;         @param responseLen Pointer to place to store the length of the response.
;     
   (AVCCommandInGeneration (:pointer :callback));(IOReturn (void * self , UInt32 busGeneration , const UInt8 * command , UInt32 cmdLen , UInt8 * response , UInt32 * responseLen))
                                                ; !	
...
;     
   (getAncestorInterface (:pointer :callback))  ;(void * (void * self , char * object_class , REFIID pluginType , REFIID iid))
                                                ; !	
...
;         @result Returns a COM-style interface pointer. Returns 0 upon failure.
;     
   (getProtocolInterface (:pointer :callback))  ;(void * (void * self , REFIID pluginType , REFIID iid))
                                                ; 
; 	
   (getAsyncConnectionPlugCounts (:pointer :callback));(IOReturn (void * self , UInt8 * inputPlugCount , UInt8 * outputPlugCount))
                                                ; 
; 	
   (** (:pointer :callback))                    ;(IUnknownVTbl (* createConsumerPlug) (void * self , UInt8 plugNumber , REFIID iid))
                                                ; !
...
;     
   (updateAVCCommandTimeout (:pointer :callback));(IOReturn (void * self))
                                                ; !
...
;     
   (makeP2PInputConnection (:pointer :callback));(IOReturn (void * self , UInt32 inputPlug , UInt32 chan))
                                                ; !
;         @function breakLocalP2PInputConnection
;         @abstract Decrements the point-to-point connection count of a unit input plug.
;         @discussion This function is only available if the interface version is > 3.
;     
   (breakP2PInputConnection (:pointer :callback));(IOReturn (void * self , UInt32 inputPlug))
                                                ; !
;         @function makeLocalP2POutputConnection
;         @abstract Increments the point-to-point connection count of a unit output plug.
;         @discussion This function is only available if the interface version is > 3.
;     
   (makeP2POutputConnection (:pointer :callback));(IOReturn (void * self , UInt32 outputPlug , UInt32 chan , IOFWSpeed speed))
                                                ; !
;         @function breakLocalP2POutputConnection
;         @abstract Decrements the point-to-point connection count of a unit output plug.
;         @discussion This function is only available if the interface version is > 3.
;     
   (breakP2POutputConnection (:pointer :callback));(IOReturn (void * self , UInt32 outputPlug))
)
; !
...
; 
(defrecord _IOFireWireAVCLibProtocolInterface
#|
   (NIL :iunknown_c_guts)|#
   (version :UInt16)
   (revision :UInt16)
...
;     
   (setAVCRequestCallback (:pointer :callback)) ;(IOReturn (void * self , UInt32 subUnitType , UInt32 subUnitID , void * refCon , IOFWAVCRequestCallback callback))
                                                ; !
...
; 
   (allocateInputPlug (:pointer :callback))     ;(IOReturn (void * self , void * refcon , IOFWAVCPCRCallback func , UInt32 * plug))
                                                ; !
...
; 
   (freeInputPlug (:pointer :callback))         ;(void (void * self , UInt32 plug))
                                                ; !
...
;     @param plug Value returned by allocateInputPlug.
; 
   (readInputPlug (:pointer :callback))         ;(UInt32 (void * self , UInt32 plug))
                                                ; !
...
; 
   (updateInputPlug (:pointer :callback))       ;(IOReturn (void * self , UInt32 plug , UInt32 oldVal , UInt32 newVal))
                                                ; !
...
;     @param plug Set to the plug number if a plug is successfully allocated.
; 
   (allocateOutputPlug (:pointer :callback))    ;(IOReturn (void * self , void * refcon , IOFWAVCPCRCallback func , UInt32 * plug))
                                                ; !
...
; 
   (freeOutputPlug (:pointer :callback))        ;(void (void * self , UInt32 plug))
                                                ; !
...
;     @param plug Value returned by allocateOutputPlug.
; 
   (readOutputPlug (:pointer :callback))        ;(UInt32 (void * self , UInt32 plug))
                                                ; !
...
;     @param newVal New value to store in plug if its current value is oldVal.
; 
   (updateOutputPlug (:pointer :callback))      ;(IOReturn (void * self , UInt32 plug , UInt32 oldVal , UInt32 newVal))
                                                ; !
;     @function readOutputMasterPlug
;     @abstract Returns the current value of the output master plug.
;     @param self Pointer to IOFireWireAVCLibProtocolInterface.
; 
   (readOutputMasterPlug (:pointer :callback))  ;(UInt32 (void * self))
                                                ; !
...
;     @param newVal New value to store in plug if its current value is oldVal.
; 
   (updateOutputMasterPlug (:pointer :callback));(IOReturn (void * self , UInt32 oldVal , UInt32 newVal))
                                                ; !
...
;     
; 
   (readInputMasterPlug (:pointer :callback))   ;(UInt32 (void * self))
                                                ; !
...
;     @param newVal New value to store in plug if its current value is oldVal.
; 
   (updateInputMasterPlug (:pointer :callback)) ;(IOReturn (void * self , UInt32 oldVal , UInt32 newVal))
                                                ; !
;     @function publishAVCUnitDirectory
;     @abstract Publishes an AVC unit directory in the config ROM.
;     @param self Pointer to IOFireWireAVCLibProtocolInterface.
; 
   (publishAVCUnitDirectory (:pointer :callback));(IOReturn (void * self))
                                                ; !
...
; 
   (installAVCCommandHandler (:pointer :callback));(IOReturn (void * self , UInt32 subUnitTypeAndID , UInt32 opCode , void * refCon , IOFWAVCCommandHandlerCallback callback))
                                                ; !
...
; 
   (sendAVCResponse (:pointer :callback))       ;(IOReturn (void * self , UInt32 generation , UInt16 nodeID , const char * response , UInt32 responseLen))
                                                ; !
;     @function addSubunit
...
;  
   (addSubunit (:pointer :callback))            ;(IOReturn (void * self , UInt32 subunitType , UInt32 numSourcePlugs , UInt32 numDestPlugs , void * refCon , IOFWAVCSubunitPlugHandlerCallback callback , UInt32 * pSubunitTypeAndID))
                                                ; !
...
; 
   (setSubunitPlugSignalFormat (:pointer :callback));(IOReturn (void * self , UInt32 subunitTypeAndID , IOFWAVCPlugTypes plugType , UInt32 plugNum , UInt32 signalFormat))
                                                ; !
...
; 
   (getSubunitPlugSignalFormat (:pointer :callback));(IOReturn (void * self , UInt32 subunitTypeAndID , IOFWAVCPlugTypes plugType , UInt32 plugNum , UInt32 * pSignalFormat))
                                                ; !
...
; 
   (connectTargetPlugs (:pointer :callback))    ;(IOReturn (void * self , UInt32 sourceSubunitTypeAndID , IOFWAVCPlugTypes sourcePlugType , UInt32 * pSourcePlugNum , UInt32 destSubunitTypeAndID , IOFWAVCPlugTypes destPlugType , UInt32 * pDestPlugNum , bool lockConnection , bool permConnection))
                                                ; !
...
; 
   (disconnectTargetPlugs (:pointer :callback)) ;(IOReturn (void * self , UInt32 sourceSubunitTypeAndID , IOFWAVCPlugTypes sourcePlugType , UInt32 sourcePlugNum , UInt32 destSubunitTypeAndID , IOFWAVCPlugTypes destPlugType , UInt32 destPlugNum))
                                                ; !
...
; 
   (getTargetPlugConnection (:pointer :callback));(IOReturn (void * self , UInt32 subunitTypeAndID , IOFWAVCPlugTypes plugType , UInt32 plugNum , UInt32 * pConnectedSubunitTypeAndID , IOFWAVCPlugTypes * pConnectedPlugType , UInt32 * pConnectedPlugNum , bool * pLockConnection , bool * pPermConnection))
)
(%define-record :IOFireWireAVCLibProtocolInterface (find-record-descriptor :_IOFIREWIREAVCLIBPROTOCOLINTERFACE))

(def-mactype :IOFireWireAVCPortStateHandler (find-mactype ':pointer)); (void * refcon , UInt32 state)

(def-mactype :IOFireWireAVCFrameStatusHandler (find-mactype ':pointer)); (void * refcon , UInt32 mode , UInt32 count)
; !
...
; 
(defrecord IOFireWireAVCLibConsumerInterface #|
   (NIL :iunknown_c_guts)|#
   (version :UInt16)
   (revision :UInt16)
   (setSubunit (:pointer :callback))            ;(void (void * self , UInt8 subunit))
   (setRemotePlug (:pointer :callback))         ;(void (void * self , UInt8 plugNumber))
   (connectToRemotePlug (:pointer :callback))   ;(IOReturn (void * self))
   (disconnectFromRemotePlug (:pointer :callback));(IOReturn (void * self))
   (setFrameStatusHandler (:pointer :callback)) ;(void (void * self , void * refcon , IOFireWireAVCFrameStatusHandler handler))
   (frameProcessed (:pointer :callback))        ;(void (void * self , UInt32 mode))
   (setMaxPayloadSize (:pointer :callback))     ;(void (void * self , UInt32 size))
   (setSegmentSize (:pointer :callback))        ;(IOReturn (void * self , UInt32 size))
   (getSegmentSize (:pointer :callback))        ;(UInt32 (void * self))
   (getSegmentBuffer (:pointer :callback))      ;(char * (void * self))
   (setPortStateHandler (:pointer :callback))   ;(void (void * self , void * refcon , IOFireWireAVCPortStateHandler handler))
   (setPortFlags (:pointer :callback))          ;(void (void * self , UInt32 flags))
   (clearPortFlags (:pointer :callback))        ;(void (void * self , UInt32 flags))
   (getPortFlags (:pointer :callback))          ;(UInt32 (void * self))
)

; #endif


(provide-interface "IOFireWireAVCLib")