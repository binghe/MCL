(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:CFHost.h"
...
(def-mactype :CFHostRef (find-mactype '(:pointer :__CFHost)))
; 
...
;  
(def-mactype :kCFStreamErrorDomainNetDB (find-mactype ':SInt32)); AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
; 
...
;  *    Non-Carbon CFM:   not available
;  
(def-mactype :kCFStreamErrorDomainSystemConfiguration (find-mactype ':SInt32)); AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
; 
;  *  CFHostInfoType
;  *  
;  *  Discussion:
...
;  
(def-mactype :CFHostInfoType (find-mactype ':sint32))
; 
;    * Results value is a CFArray of CFData's (each being a struct
;    * sockaddr)
;    

(defconstant $kCFHostAddresses 0)
; 
;    * Results value is a CFArray of CFString's
;    

(defconstant $kCFHostNames 1)
; 
;    * Results value is a CFData wrapping SCNetworkConnectionFlags
;    * (defined in SystemConfiguration/SCNetwork.h)
;    

(defconstant $kCFHostReachability 2)

;type name? (def-mactype :CFHostInfoType (find-mactype ':CFHostInfoType))
; 
;  *  CFHostClientContext
;  *  
;  *  Discussion:
...
;type name? (%define-record :CFHostClientContext (find-record-descriptor ':CFHostClientContext))
; 
...
(def-mactype :CFHostClientCallBack (find-mactype ':pointer)); (CFHostRef theHost , CFHostInfoType typeInfo , const CFStreamError * error , void * info)
; 
;  *  CFHostGetTypeID()
;  *  
;  *  Discussion:
...
(deftrap-inline "_CFHostGetTypeID" 
   (
...
(deftrap-inline "_CFHostCreateWithName" 
...
(deftrap-inline "_CFHostCreateWithAddress" 
   ((allocator (:pointer :__CFAllocator))
    (addr (:pointer :__CFData))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   (:pointer :__CFHost)
() )
; 
...
(deftrap-inline "_CFHostCreateCopy" 
...
(deftrap-inline "_CFHostStartInfoResolution" 
   ((theHost (:pointer :__CFHost))
    (info :CFHostInfoType)
...
(deftrap-inline "_CFHostGetAddressing" 
   ((theHost (:pointer :__CFHost))
    (hasBeenResolved (:pointer :Boolean))       ;  can be NULL 
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   (:pointer :__CFArray)
() )
; 
...
(deftrap-inline "_CFHostGetNames" 
   ((theHost (:pointer :__CFHost))
    (hasBeenResolved (:pointer :Boolean))       ;  can be NULL 
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   (:pointer :__CFArray)
...
(deftrap-inline "_CFHostGetReachability" 
   ((theHost (:pointer :__CFHost))
    (hasBeenResolved (:pointer :Boolean))       ;  can be NULL 
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   (:pointer :__CFData)
...
(deftrap-inline "_CFHostCancelInfoResolution" 
   ((theHost (:pointer :__CFHost))
    (info :CFHostInfoType)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   nil
...
(deftrap-inline "_CFHostSetClient" 
   ((theHost (:pointer :__CFHost))
    (clientCB :pointer)                         ;  can be NULL 
    (clientContext (:pointer :CFHostClientContext));  can be NULL 
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :Boolean
() )
; 
...
(deftrap-inline "_CFHostScheduleWithRunLoop" 
   ((theHost (:pointer :__CFHost))
...
(deftrap-inline "_CFHostUnscheduleFromRunLoop" 
   ((theHost (:pointer :__CFHost))
    (runLoop (:pointer :__CFRunLoop))
    (runLoopMode (:pointer :__CFString))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
