(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:Pasteboard.h"
...
(def-mactype :PasteboardRef (find-mactype '(:pointer :OpaquePasteboardRef)))

(def-mactype :PasteboardItemID (find-mactype '(:pointer :void)))
;  Pasteboard Manager error codes

(defconstant $badPasteboardSyncErr -25130)      ;  pasteboard has been modified and must be synchronized

(defconstant $badPasteboardIndexErr -25131)     ;  item index does not exist

(defconstant $badPasteboardItemErr -25132)      ;  item reference does not exist

(defconstant $badPasteboardFlavorErr -25133)    ;  item flavor does not exist

(defconstant $duplicatePasteboardFlavorErr -25134);  item flavor already exists

(defconstant $notPasteboardOwnerErr -25135)     ;  client did not clear the pasteboard
;  a promise is being added without a promise keeper

(defconstant $noPasteboardPromiseKeeperErr -25136)
; 
;  *  PasteboardSyncFlags
;  *  
;  *  Summary:
...
(def-mactype :PasteboardSyncFlags (find-mactype ':UInt32))
; 
...
(defconstant $kPasteboardModified 1)
; 
...
(defconstant $kPasteboardClientIsOwner 2)
; 
...
(def-mactype :PasteboardFlavorFlags (find-mactype ':UInt32))
; 
;    * No additional information exists for this flavor.
;    

(defconstant $kPasteboardFlavorNoFlags 0)
; 
...
(defconstant $kPasteboardFlavorSenderOnly 1)
; 
...
(defconstant $kPasteboardFlavorSenderTranslated 2)
; 
...
(defconstant $kPasteboardFlavorNotSaved 4)
; 
...
(defconstant $kPasteboardFlavorRequestOnly 8)
; 
...
(defconstant $kPasteboardFlavorSystemTranslated #x100)
; 
...
(defconstant $kPasteboardFlavorPromised #x200)
; 
;  *  PasteboardGetTypeID()
;  *  
;  *  Summary:
...
(deftrap-inline "_PasteboardGetTypeID" 
   (
...
(deftrap-inline "_PasteboardCreate" 
...
(deftrap-inline "_PasteboardSynchronize" 
...
(deftrap-inline "_PasteboardClear" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_PasteboardGetItemCount" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (outItemCount (:pointer :ItemCount))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PasteboardGetItemIdentifier" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inIndex :UInt32)
    (outItem (:pointer :PASTEBOARDITEMID))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PasteboardCopyItemFlavors" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inItem (:pointer :void))
    (outFlavorTypes (:pointer :CFArrayRef))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PasteboardGetItemFlavorFlags" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inItem (:pointer :void))
    (inFlavorType (:pointer :__CFString))
    (outFlags (:pointer :PASTEBOARDFLAVORFLAGS))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PasteboardCopyItemFlavorData" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inItem (:pointer :void))
    (inFlavorType (:pointer :__CFString))
...
(deftrap-inline "_PasteboardPutItemFlavor" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inItem (:pointer :void))
    (inFlavorType (:pointer :__CFString))
    (inData (:pointer :__CFData))               ;  can be NULL 
    (inFlags :UInt32)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_PasteboardCopyPasteLocation" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (outPasteLocation (:pointer :CFURLRef))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PasteboardSetPasteLocation" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inPasteLocation (:pointer :__CFURL))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
; 
...
(def-mactype :PasteboardPromiseKeeperProcPtr (find-mactype ':pointer)); (PasteboardRef pasteboard , PasteboardItemID item , CFStringRef flavorType , void * context)
; 
...
(deftrap-inline "_PasteboardSetPromiseKeeper" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
    (inPromiseKeeper :pointer)
    (inContext :pointer)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_PasteboardResolvePromises" 
   ((inPasteboard (:pointer :OpaquePasteboardRef))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_3_AND_LATER
   :OSStatus
() )
