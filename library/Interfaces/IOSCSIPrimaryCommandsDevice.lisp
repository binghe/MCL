(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOSCSIPrimaryCommandsDevice.h"
; at Sunday July 2,2006 7:29:51 pm.
; 
...
(require-interface "IOKit/scsi/SCSICommandDefinitions")
...
(require-interface "IOKit/scsi/IOSCSIProtocolInterface")
...
(defconstant $kSCSIServicesNotification_Suspend #x69000080)
(defconstant $kSCSIServicesNotification_Resume #x69000090)
;  Timeout values

(defconstant $kOneSecondTimeoutInMS #x3E8)
(defconstant $kTenSecondTimeoutInMS #x2710)
(defconstant $kThirtySecondTimeoutInMS #x7530)
;  Mode page values for page control field

(defconstant $kModePageControlCurrentValues 0)
(defconstant $kModePageControlChangeableValues 1)
(defconstant $kModePageControlDefaultValues 2)
(defconstant $kModePageControlSavedValues 3)
...
; ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
#|
 confused about CLASS IOSCSIPrimaryCommandsDevice #\: public IOSCSIProtocolInterface #\{ OSDeclareAbstractStructors #\( IOSCSIPrimaryCommandsDevice #\) private #\: SCSIPrimaryCommands * fSCSIPrimaryCommandObject #\; IOSCSIProtocolInterface * fProtocolDriver #\;;  The fProtocolAccessEnabled member variable indicates whether
...
;  TerminateDeviceSupport is called.
 bool fProtocolAccessEnabled #\;                ;  The fDeviceAccessEnabled member variable indicates whether
;  requests coming from the client can be sent to the device.
;  This will be set to true after InitializeDeviceSupport is called and
;  set to false before StopDeviceSupport is called.
 bool fDeviceAccessEnabled #\; bool fDeviceAccessSuspended #\; static bool ServerKeyswitchCallback #\( void * target #\, void * refCon #\, IOService * newService #\) #\; static void TaskCallback #\( SCSITaskIdentifier completedTask #\) #\; protected #\:;  Reserve space for future expansion.
 struct IOSCSIPrimaryCommandsDeviceExpansionData #\{ IONotifier * fKeySwitchNotifier #\; UInt8 fANSIVersion #\; UInt32 fReadTimeoutDuration #\; UInt32 fWriteTimeoutDuration #\; bool fCMDQUE #\; SCSITaggedTaskIdentifier fTaskID #\; IOSimpleLock * fTaskIDLock #\; #\} #\; IOSCSIPrimaryCommandsDeviceExpansionData * fIOSCSIPrimaryCommandsDeviceReserved #\;
...
;  objects will be removed in a later release.
 virtual SCSIPrimaryCommands * GetSCSIPrimaryCommandObject #\( void #\) #\;;  This method is called by the start method to create all the command
;  objects needed by the class.  For subclasses, this will be overridden
;  to create its needed command set objects.
 virtual bool CreateCommandSetObjects #\( void #\) #\;;  This method is called by the free method to free all the command
;  objects needed by the class.  For subclasses, this will be overridden
;  to free its needed command set objects.
 virtual void FreeCommandSetObjects #\( void #\) #\;;  This method is called by the start method to obtain information from
;  the device with regards to whether it supports the power conditions mode page.
 virtual void CheckPowerConditionsModePage #\( void #\) #\;;  This will return back the Protocol driver that is used to
;  send service requests to the device.
 virtual IOSCSIProtocolInterface * GetProtocolDriver #\( void #\) #\;;  This will get a new SCSITask for the caller
 virtual SCSITaskIdentifier GetSCSITask #\( void #\) #\;;  This will release a SCSITask (eventually return it to a pool)
 virtual void ReleaseSCSITask #\( SCSITaskIdentifier request #\) #\;;  This will return a unique value for the tagged task identifier
 SCSITaggedTaskIdentifier GetUniqueTagID #\( void #\) #\;;  Call for executing the command synchronously	
 SCSIServiceResponse SendCommand #\( SCSITaskIdentifier request #\, UInt32 timeoutDuration #\) #\;;  Call for executing the command asycnchronously	
 void SendCommand #\( SCSITaskIdentifier request #\, UInt32 timeoutDuration #\, SCSITaskCompletion taskCompletion #\) #\; virtual bool InitializeDeviceSupport #\( void #\) = 0 #\; virtual void StartDeviceSupport #\( void #\) = 0 #\; virtual void SuspendDeviceSupport #\( void #\) = 0 #\; virtual void ResumeDeviceSupport #\( void #\) = 0 #\; virtual void StopDeviceSupport #\( void #\) = 0 #\; virtual void TerminateDeviceSupport #\( void #\) = 0 #\; virtual UInt32 GetNumberOfPowerStateTransitions #\( void #\) = 0 #\; virtual IOReturn VerifyDeviceState #\( void #\) #\;;  Called to clear any power-on/reset status in the drive
 virtual bool ClearPowerOnReset #\( void #\) #\; virtual bool ClearNotReadyStatus #\( void #\) = 0 #\; bool IsProtocolAccessEnabled #\( void #\) #\; bool IsDeviceAccessEnabled #\( void #\) #\; bool IsDeviceAccessSuspended #\( void #\) #\;;  Accessors for saving and retrieving data from an SCSITask object.
 bool ResetForNewTask #\( SCSITaskIdentifier request #\) #\; bool SetTaskAttribute #\( SCSITaskIdentifier request #\, SCSITaskAttribute newAttribute #\) #\; SCSITaskAttribute GetTaskAttribute #\( SCSITaskIdentifier request #\) #\; bool SetTaggedTaskIdentifier #\( SCSITaskIdentifier request #\, SCSITaggedTaskIdentifier taggedTaskIdentifier #\) #\; SCSITaggedTaskIdentifier GetTaggedTaskIdentifier #\( SCSITaskIdentifier request #\) #\; bool SetTaskState #\( SCSITaskIdentifier request #\, SCSITaskState newTaskState #\) #\; SCSITaskState GetTaskState #\( SCSITaskIdentifier request #\) #\; bool SetTaskStatus #\( SCSITaskIdentifier request #\, SCSITaskStatus newStatus #\) #\; SCSITaskStatus GetTaskStatus #\( SCSITaskIdentifier request #\) #\; bool SetCommandDescriptorBlock #\( SCSITaskIdentifier request #\, UInt8 cdbByte0 #\, UInt8 cdbByte1 #\, UInt8 cdbByte2 #\, UInt8 cdbByte3 #\, UInt8 cdbByte4 #\, UInt8 cdbByte5 #\) #\;;  Populate the 10 Byte Command Descriptor Block
 bool SetCommandDescriptorBlock #\( SCSITaskIdentifier request #\, UInt8 cdbByte0 #\, UInt8 cdbByte1 #\, UInt8 cdbByte2 #\, UInt8 cdbByte3 #\, UInt8 cdbByte4 #\, UInt8 cdbByte5 #\, UInt8 cdbByte6 #\, UInt8 cdbByte7 #\, UInt8 cdbByte8 #\, UInt8 cdbByte9 #\) #\;;  Populate the 12 Byte Command Descriptor Block
 bool SetCommandDescriptorBlock #\( SCSITaskIdentifier request #\, UInt8 cdbByte0 #\, UInt8 cdbByte1 #\, UInt8 cdbByte2 #\, UInt8 cdbByte3 #\, UInt8 cdbByte4 #\, UInt8 cdbByte5 #\, UInt8 cdbByte6 #\, UInt8 cdbByte7 #\, UInt8 cdbByte8 #\, UInt8 cdbByte9 #\, UInt8 cdbByte10 #\, UInt8 cdbByte11 #\) #\;;  Populate the 16 Byte Command Descriptor Block
...
;  the object that is claimed as the owner of the specified SCSITask.
 static OSObject * sGetOwnerForTask #\( SCSITaskIdentifier request #\) #\; public #\: bool init #\( OSDictionary * propTable #\) #\; virtual bool start #\( IOService * provider #\) #\; virtual void stop #\( IOService * provider #\) #\; virtual IOReturn message #\( UInt32 type #\, IOService * nub #\, void * arg #\) #\;;  The setAgressiveness method is called by the power manager
...
;  in order to set our idle timer.
 virtual IOReturn setAggressiveness #\( UInt32 type #\, UInt32 minutes #\) #\;;  Methods for getting device information strings
 virtual char * GetVendorString #\( void #\) #\; virtual char * GetProductString #\( void #\) #\; virtual char * GetRevisionString #\( void #\) #\; OSDictionary * GetProtocolCharacteristicsDictionary #\( void #\) #\; OSDictionary * GetDeviceCharacteristicsDictionary #\( void #\) #\; UInt8 GetANSIVersion #\( void #\) #\; bool GetCMDQUE #\( void #\) #\;;  -- SCSI Protocol Interface Methods	--
;  The ExecuteCommand method will take a SCSI Task and transport
;  it across the physical wire(s) to the device
 virtual void ExecuteCommand #\( SCSITaskIdentifier request #\) #\;;  The Task Management function to allow the SCSI Application Layer client to request
;  that a specific task be aborted.
 SCSIServiceResponse AbortTask #\( UInt8 theLogicalUnit #\, SCSITaggedTaskIdentifier theTag #\) #\;;  The Task Management function to allow the SCSI Application Layer client to request
;  that a all tasks curerntly in the task set be aborted.
 SCSIServiceResponse AbortTaskSet #\( UInt8 theLogicalUnit #\) #\; SCSIServiceResponse ClearACA #\( UInt8 theLogicalUnit #\) #\; SCSIServiceResponse ClearTaskSet #\( UInt8 theLogicalUnit #\) #\; SCSIServiceResponse LogicalUnitReset #\( UInt8 theLogicalUnit #\) #\; SCSIServiceResponse TargetReset #\( void #\) #\;;  The AbortCommand method will abort the indicated SCSI Task,
;  if it is possible and the task has not already completed.
 virtual SCSIServiceResponse AbortCommand #\( SCSITaskIdentifier request #\) #\;;  The IsProtocolServiceSupported will return true if the specified
...
;  returned, it will be returned in the serviceValue output parameter.
 virtual bool IsProtocolServiceSupported #\( SCSIProtocolFeature feature #\, void * serviceValue #\) #\;;  The HandleProtocolServiceFeature will return true if the specified feature could
;  be handled properly by the protocol layer.
 virtual bool HandleProtocolServiceFeature #\( SCSIProtocolFeature feature #\, void * serviceValue #\) #\;;  Utility methods for use by all peripheral device objects.
...
(provide-interface "IOSCSIPrimaryCommandsDevice")