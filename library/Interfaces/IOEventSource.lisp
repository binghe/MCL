(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOEventSource.h"
...
; 
#|
 confused about CLASS IOEventSource #\: public OSObject #\{ OSDeclareAbstractStructors #\( IOEventSource #\) friend class IOWorkLoop #\; public #\:
; !
;     @typedef Action
...
; 	The next event source in the event chain. nil at end of chain. 
 IOEventSource * eventChainNext #\;             ; ! @var owner The owner object called when an event has been delivered. 
 OSObject * owner #\;
; ! @var action
...
; 	Is this event source enabled to deliver requests to the work-loop. 
 bool enabled #\;                               ; ! @var workLoop What is the work-loop for this event source. 
 IOWorkLoop * workLoop #\;                      ; ! @var refcon What ever the client wants to do, see $link setRefcon. 
 void * refcon #\;
...
; 
 virtual IOEventSource * getNext #\( #\) const #\; protected #\:;  Methods to access the IOWorkLoop exported fields
;  inline 
 void signalWorkAvailable #\( #\) #\;           ;  { workLoop->signalWorkAvailable(); }; 
;  inline 
 void openGate #\( #\) #\;                      ;  { workLoop->openGate(); }; 
;  inline 
 void closeGate #\( #\) #\;                     ;  { workLoop->closeGate(); }; 
;  inline 
 bool tryCloseGate #\( #\) #\;                  ;  { return workLoop->tryCloseGate(); }; 
;  inline 
 int sleepGate #\( void * event #\, UInt32 type #\) #\;;  { return workLoop->sleepGate(event, type); }; 
;  inline 
 void wakeupGate #\( void * event #\, bool oneThread #\) #\;;  { workLoop->wakeupGate(event, oneThread); }; 
 public #\:
; ! @function setAction
...
; 
 virtual bool onThread #\( #\) const #\; private #\: OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 0 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 1 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 2 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 3 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 4 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 5 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 6 #\) #\; OSMetaClassDeclareReservedUnused #\( IOEventSource #\, 7 #\) #\;
|#

; #endif /* !_IOKIT_IOEVENTSOURCE_H */


(provide-interface "IOEventSource")