(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:Debugging.h"
...
(defconstant $kComponentSignatureString "Third Party Client")
; #define kComponentSignatureString "Third Party Client"

; #endif

; #ifndef COMPONENT_SIGNATURE
(defconstant $COMPONENT_SIGNATURE "?*?*")
...
(defconstant $DEBUG_LEVEL_BREAK_ONLY 1)
; #define DEBUG_LEVEL_BREAK_ONLY    1
(defconstant $DEBUG_LEVEL_EXTERNAL 3)
; #define DEBUG_LEVEL_EXTERNAL  3
(defconstant $DEBUG_LEVEL_INTERNAL 4)
...
;  
(defconstant $DEBUG_NO_OPTIONS 0)
...
; #define DEBUGASSERTMSG(componentSignature, options, assertionString, exceptionLabelString, errorString, fileName, lineNumber, value)          Debugger()

; #elif DEBUGLEVEL == DEBUG_LEVEL_EXTERNAL
;  exclude fileName and lineNumber 
; #define DEBUGASSERTMSG(componentSignature, options, assertionString, exceptionLabelString, errorString, fileName, lineNumber, value)          DebugAssert(componentSignature, options, assertionString, exceptionLabelString, errorString, 0, 0, (void*)value)
 |#

; #else
;  include all info 
; #define DEBUGASSERTMSG(componentSignature, options, assertionString, exceptionLabelString, errorString, fileName, lineNumber, value)          DebugAssert(componentSignature, options, assertionString, exceptionLabelString, errorString, fileName, lineNumber, (void*)value)
...
(require-interface "AssertMacros")
; 
...
(defconstant $kBlessedBusErrorBait #x68F168F1)
; 
;  *  DebugAssert()
;  *  
;  *  Summary:
...
(deftrap-inline "_DebugAssert" 
   ((componentSignature :OSType)
    (options :UInt32)
    (assertionString (:pointer :char))
    (exceptionLabelString (:pointer :char))
    (errorString (:pointer :char))
    (fileName (:pointer :char))
    (lineNumber :signed-long)
    (value :pointer)
...
(defconstant $k68kInterruptLevelMask 7)         ;  68K interrupt levels 0 through 7 

(defconstant $kInVBLTaskMask 16)                ;  VBLs are executing 

(defconstant $kInDeferredTaskMask 32)           ;  Deferred tasks are executing 

(defconstant $kInSecondaryIntHandlerMask 64)    ;  Secondary interrupt handlers are executing 

(defconstant $kInNestedInterruptMask #x80)      ;  The system is handling an interrupt 
...
(defconstant $kComponentDebugOption 0)          ;  optionSelectorNum to turn breaks for component On or Off


(defconstant $kGetDebugOption 1)                ;  get current debug option setting

(defconstant $kSetDebugOption 2)                ;  set debug option
...
(def-mactype :DebugComponentCallbackProcPtr (find-mactype ':pointer)); (SInt32 optionSelectorNum , UInt32 command , Boolean * optionSetting)

(def-mactype :DebugComponentCallbackUPP (find-mactype '(:pointer :OpaqueDebugComponentCallbackProcPtr)))
; 
;  *  NewDebugComponent()
;  *  
;  *  Summary:
...
(deftrap-inline "_NewDebugComponent" 
   ((componentSignature :OSType)
    (componentName (:pointer :STR255))
    (componentCallback (:pointer :OpaqueDebugComponentCallbackProcPtr))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_NewDebugOption" 
   ((componentSignature :OSType)
    (optionSelectorNum :SInt32)
    (optionName (:pointer :STR255))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_DisposeDebugComponent" 
   ((componentSignature :OSType)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_GetDebugComponentInfo" 
   ((index :UInt32)
    (componentSignature (:pointer :OSType))
    (componentName (:pointer :STR255))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_GetDebugOptionInfo" 
   ((index :UInt32)
    (componentSignature :OSType)
    (optionSelectorNum (:pointer :SInt32))
    (optionName (:pointer :STR255))
    (optionSetting (:pointer :Boolean))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_SetDebugOptionValue" 
   ((componentSignature :OSType)
    (optionSelectorNum :SInt32)
...
(def-mactype :DebugAssertOutputHandlerProcPtr (find-mactype ':pointer)); (OSType componentSignature , UInt32 options , const char * assertionString , const char * exceptionLabelString , const char * errorString , const char * fileName , long lineNumber , void * value , ConstStr255Param outputMsg)

(def-mactype :DebugAssertOutputHandlerUPP (find-mactype '(:pointer :OpaqueDebugAssertOutputHandlerProcPtr)))
; 
...
(deftrap-inline "_InstallDebugAssertOutputHandler" 
   ((handler (:pointer :OpaqueDebugAssertOutputHandlerProcPtr))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   nil
() )
; 
...
(deftrap-inline "_NewDebugComponentCallbackUPP" 
   ((userRoutine :pointer)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   (:pointer :OpaqueDebugComponentCallbackProcPtr)
() )
; 
...
(deftrap-inline "_NewDebugAssertOutputHandlerUPP" 
   ((userRoutine :pointer)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   (:pointer :OpaqueDebugAssertOutputHandlerProcPtr)
() )
; 
...
(deftrap-inline "_DisposeDebugComponentCallbackUPP" 
   ((userUPP (:pointer :OpaqueDebugComponentCallbackProcPtr))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   nil
() )
; 
...
(deftrap-inline "_DisposeDebugAssertOutputHandlerUPP" 
   ((userUPP (:pointer :OpaqueDebugAssertOutputHandlerProcPtr))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   nil
() )
; 
...
(deftrap-inline "_InvokeDebugComponentCallbackUPP" 
   ((optionSelectorNum :SInt32)
    (command :UInt32)
    (optionSetting (:pointer :Boolean))
    (userUPP (:pointer :OpaqueDebugComponentCallbackProcPtr))
...
(deftrap-inline "_InvokeDebugAssertOutputHandlerUPP" 
   ((componentSignature :OSType)
    (options :UInt32)
    (assertionString (:pointer :char))
    (exceptionLabelString (:pointer :char))
