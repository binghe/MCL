(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOFramebuffer.h"
...
(def-mactype :CursorBlitProc (find-mactype ':pointer)); (IOFramebuffer * inst , void * shmem , volatile unsigned char * vramPtr , unsigned int cursStart , unsigned int vramRow , unsigned int cursRow , int width , int height)

(def-mactype :CursorRemoveProc (find-mactype ':pointer)); (IOFramebuffer * inst , void * shmem , volatile unsigned char * vramPtr , unsigned int vramRow , int width , int height)

(defconstant $kHardwareCursorInfoMajorVersion 1)
(defconstant $kHardwareCursorInfoMinorVersion 0)
; !
...
;  
(defrecord IOHardwareCursorInfo
   (majorVersion :UInt16)
   (minorVersion :UInt16)
   (cursorHeight :UInt32)
   (cursorWidth :UInt32)
                                                ;  nil or big enough for hardware's max colors
   (colorMap (:pointer :IOColorEntry))
   (hardwareCursorData (:pointer :UInt8))
   (reserved (:array :UInt32 6))
)

;type name? (%define-record :IOHardwareCursorInfo (find-record-descriptor ':IOHardwareCursorInfo))

(def-mactype :IOFBCursorRef (find-mactype '(:pointer :void)))
(defrecord IOFBCursorControlCallouts
   (setCursorImage (:pointer :callback))        ;(IOReturn (void * self , void * ref , IOHardwareCursorDescriptor * description , IOFBCursorRef cursorImage))
   (setCursorState (:pointer :callback))        ;(IOReturn (void * self , void * ref , SInt32 x , SInt32 y , bool visible))
   (reserved (:array :UInt32 30))
)

;type name? (%define-record :IOFBCursorControlCallouts (find-record-descriptor ':IOFBCursorControlCallouts))
(defrecord IOFBCursorControlAttribute
   (self :pointer)
   (ref :pointer)
   (callouts (:pointer :IOFBCursorControlCallouts))
   (reserved (:array :UInt32 29))
)

;type name? (%define-record :IOFBCursorControlAttribute (find-record-descriptor ':IOFBCursorControlAttribute))
;  clock & data values

(defconstant $kIODDCLow 0)
(defconstant $kIODDCHigh 1)
(defconstant $kIODDCTristate 2)
;  ddcBlockType constants
;  EDID block type.

(defconstant $kIODDCBlockTypeEDID 0)
;  ddcFlags constants
;  Force a new read of the EDID.

(defconstant $kIODDCForceRead 1)

(defconstant $kDisabledInterruptState 0)
(defconstant $kEnabledInterruptState 1)

(def-mactype :IOFBInterruptProc (find-mactype ':pointer)); (OSObject * target , void * ref)

(def-mactype :IOFramebufferNotificationHandler (find-mactype ':pointer)); (OSObject * self , void * ref , IOFramebuffer * framebuffer , IOIndex event , void * info)
;  IOFramebufferNotificationHandler events

(defconstant $kIOFBNotifyDisplayModeWillChange 1)
(defconstant $kIOFBNotifyDisplayModeDidChange 2)
(defconstant $kIOFBNotifyWillSleep 3)
(defconstant $kIOFBNotifyDidWake 4)
(defconstant $kIOFBNotifyDidPowerOff 5)
(defconstant $kIOFBNotifyWillPowerOn 6)
(defconstant $kIOFBNotifyDidSleep 5)
(defconstant $kIOFBNotifyWillWake 6)
(defconstant $kIOFBNotifyWillPowerOff 7)
(defconstant $kIOFBNotifyDidPowerOn 8)
(defconstant $kIOFBNotifyWillChangeSpeed 9)
(defconstant $kIOFBNotifyDidChangeSpeed 10)

(defconstant $kFBDisplayUsablePowerState #x80000000)
(defconstant $kFBDisplayPowerStateMask #xFFFF)
(defconstant $kIOFBDependentIDKey "IOFBDependentID")
; #define kIOFBDependentIDKey	"IOFBDependentID"
(defconstant $kIOFBDependentIndexKey "IOFBDependentIndex")
...
; 
#|
 confused about CLASS IOFramebuffer #\: public IOGraphicsDevice #\{ friend class IOFramebufferUserClient #\; friend class IOFramebufferSharedUserClient #\; friend class IOGraphicsEngineClient #\; OSDeclareDefaultStructors #\( IOFramebuffer #\) protected #\:
...
;     Reserved for future use.  (Internal use only)  
 ExpansionData * reserved #\; private #\: protected #\: StdFBShmem_t * priv #\; int shmemClientVersion #\; IOBufferMemoryDescriptor * sharedCursor #\; union #\{ struct #\{;  Mapping tables used in cursor drawing to 5-5-5 displays. 
 unsigned char * _bm34To35SampleTable #\; unsigned char * _bm35To34SampleTable #\;;  Mapping tables used in cursor drawing to 8-bit RGB displays. 
 unsigned int * _bm256To38SampleTable #\; unsigned char * _bm38To256SampleTable #\; #\} t #\; UInt8 * tables #\[ 4 #\] #\; #\} colorConvert #\;;  cursor blitting vars 
 CursorBlitProc cursorBlitProc #\; CursorRemoveProc cursorRemoveProc #\; IOGSize maxCursorSize #\; void * _IOFramebuffer_reservedE #\[ 8 #\] #\; volatile unsigned char * cursorSave #\; unsigned int white #\; Point nextCursorLoc #\; int nextCursorFrame #\; SInt32 connectChange #\; semaphore_t vblSemaphore #\;;  memory ranges 
 volatile unsigned char * frameBuffer #\; unsigned int totalWidth #\; unsigned int rowBytes #\; unsigned int bytesPerPixel #\; IOMemoryMap * vramMap #\; IOByteCount vramMapOffset #\; OSArray * userAccessRanges #\; unsigned int suspended #\: 1 #\; unsigned int captured #\: 1 #\; unsigned int sleepConnectCheck #\: 1 #\; unsigned int messaged #\: 1 #\; unsigned int _IOFramebuffer_reservedC #\: 28 #\; IOFramebuffer * nextDependent #\; OSSet * fbNotifications #\; class IOFramebufferUserClient * serverConnect #\; class IOFramebufferSharedUserClient * sharedConnect #\; unsigned int opened #\: 1 #\; unsigned int dead #\: 1 #\; unsigned int configPending #\: 1 #\; unsigned int serverNotified #\: 1 #\; unsigned int serverState #\: 1 #\; unsigned int serverPendingAck #\: 1 #\; unsigned int isUsable #\: 1 #\; unsigned int mirrored #\: 1 #\; unsigned int pendingPowerState #\: 4 #\; unsigned int pendingPowerChange #\: 1 #\; unsigned int pagingState #\: 1 #\; unsigned int mirrorPrimary #\: 1 #\; unsigned int mirrorSWCursor #\: 1 #\; bool clutValid #\; bool currentMono #\; bool needCursorService #\; bool haveVBLService #\; bool haveHWCursor #\; bool hwCursorLoaded #\; void * serverMsg #\; IOInterruptEventSource * deferredEvents #\;;  Reserved for future expansion. 
 int _IOFramebuffer_reserved #\[ 5 #\] #\; private #\: struct IOFramebufferPrivate * __private #\; public #\:
...
;     @result an IODeviceMemory instance. A reference will be consumed by the caller for each call of this method - the implementatation should create a new instance of IODeviceMemory for each call, or return one instance with a retain for each call.
; 
 virtual IODeviceMemory * getVRAMRange #\( void #\) #\; protected #\:
...
; 
 IOReturn handleEvent #\( IOIndex event #\, void * info = 0 #\) #\; IOReturn deliverFramebufferNotification #\( IOIndex event #\, void * info = 0 #\) #\;; #ifdef IOFRAMEBUFFER_PRIVATE
#| #|
#include <IOKitgraphicsIOFramebufferPrivate.h>
#endif
|#
 |#
 public #\:
...
;     @result An IOReturn code.
; 
 virtual IOReturn connectFlags #\( IOIndex connectIndex #\, IODisplayModeID displayMode #\, IOOptionBits * flags #\) #\;; // IOLowLevelDDCSense
...
; 
 virtual bool readDDCData #\( IOIndex bus #\) #\; virtual IOReturn enableDDCRaster #\( bool enable #\) #\;
...
;     @result An IOReturn code.
; 
 virtual IOReturn setInterruptState #\( void * interruptRef #\, UInt32 state #\) #\; virtual IOReturn getNotificationSemaphore #\( IOSelect interruptType #\, semaphore_t * semaphore #\) #\;
|#

; #endif /* ! _IOKIT_IOFRAMEBUFFER_H */


(provide-interface "IOFramebuffer")