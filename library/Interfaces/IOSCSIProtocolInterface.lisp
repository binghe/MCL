(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOSCSIProtocolInterface.h"
; at Sunday July 2,2006 7:29:52 pm.
; 
...
(require-interface "IOKit/storage/IOStorageDeviceCharacteristics")
; ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
; 	Constants
; ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
...
(defconstant $kIOPropertySCSIDeviceCharacteristicsKey "SCSI Device Characteristics")
...
;  command.
(defconstant $kIOPropertySCSIInquiryLengthKey "Inquiry Length")
...
(defconstant $kIOPropertySCSIManualEjectKey "Manual Eject")
...
(defconstant $kIOPropertyReadTimeOutDurationKey "Read Time Out Duration")
...
;  This property overrides all of the protocol defaults
;  This Property is a value, in miliseconds
(defconstant $kIOPropertyWriteTimeOutDurationKey "Write Time Out Duration")
...
(defconstant $kSCSIProtocolNotification_DeviceRemoved #x69000010)
(defconstant $kSCSIProtocolNotification_VerifyDeviceState #x69000020)
(defconstant $kSCSIServicesNotification_ExclusivityChanged #x69000030)

(def-mactype :SCSIProtocolFeature (find-mactype ':UInt32))
;  SCSI Protocol Features
;  kSCSIProtocolFeature_ACA:
;  Not yet used.

(defconstant $kSCSIProtocolFeature_ACA 1)       ;  kSCSIProtocolFeature_CPUInDiskMode:
;  Used to determine if the SCSI Protocol Services Driver supports
;  a CPU which is in target disk mode.

(defconstant $kSCSIProtocolFeature_CPUInDiskMode 2);  kSCSIProtocolFeature_ProtocolSpecificPolling:
;  Used to determine if the SCSI Protocol Services Driver supports
;  protocol specific polling for media. This is used for low-power
;  polling specifically for ATAPI devices on ATA buses.

(defconstant $kSCSIProtocolFeature_ProtocolSpecificPolling 3);  kSCSIProtocolFeature_ProtocolSpecificSleepCommand:
;  Used to determine if the SCSI Protocol Services Driver supports
;  protocol specific sleep commands to a drive. This is used for
;  sleeping drives specifically ATAPI devices on ATA buses.

(defconstant $kSCSIProtocolFeature_ProtocolSpecificSleepCommand 4);  kSCSIProtocolFeature_GetMaximumLogicalUnitNumber:
...
(defconstant $kSCSIProtocolFeature_GetMaximumLogicalUnitNumber 5);  kSCSIProtocolFeature_MaximumReadBlockTransferCount and 
...
(defconstant $kSCSIProtocolFeature_MaximumReadBlockTransferCount 6)
(defconstant $kSCSIProtocolFeature_MaximumWriteBlockTransferCount 7);  kSCSIProtocolFeature_MaximumReadTransferByteCount and 
...
(defconstant $kSCSIProtocolFeature_MaximumReadTransferByteCount 8)
(defconstant $kSCSIProtocolFeature_MaximumWriteTransferByteCount 9);  kSCSIProtocolFeature_SubmitDefaultInquiryData: 
...
(defconstant $kSCSIProtocolFeature_SubmitDefaultInquiryData 10)
; ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
; 	Class Declaration
; ÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑÑ
...
#|
 confused about CLASS IOSCSIProtocolInterface #\: public IOService #\{ OSDeclareAbstractStructors #\( IOSCSIProtocolInterface #\) public #\: virtual bool start #\( IOService * provider #\) #\; virtual void free #\( void #\) #\;;  ------ User Client Support ------
...
;  current time.
 virtual bool GetUserClientExclusivityState #\( void #\) #\;;  The SetUserClientExclusivityState method is called by the user client
;  to set the exclusive access mode.
 virtual IOReturn SetUserClientExclusivityState #\( IOService * userClient #\, bool state #\) #\;;  ------ Power Management Support ------
...
;  in the power tree.
 virtual UInt32 initialPowerStateForDomainState #\( IOPMPowerFlags flags #\) #\;;  The setPowerState method is called by the power manager to tell us
...
;  drive.
 virtual IOReturn setPowerState #\( UInt32 powerStateOrdinal #\, IOService * whichDevice #\) #\;;  The IsPowerManagementIntialized method can be called to determine whether or not
;  power management has been successfully initialized.
 virtual bool IsPowerManagementIntialized #\( void #\) #\;;  The CheckPowerState method is called by clients of the object before
...
;  device is ready.
 virtual void CheckPowerState #\( void #\) #\;  ;  -- Functionality available to the SCSI Application Layer	--
;  The ExecuteCommand method will take a SCSI Task and transport
;  it across the physical wire(s) to the device
 virtual void ExecuteCommand #\( SCSITaskIdentifier request #\) = 0 #\;;  ************* Obsoleted Member Routine ****************
;  The AbortCommand method is replaced by the AbortTask Management function and
;  should no longer be called.
 virtual SCSIServiceResponse AbortCommand #\( SCSITaskIdentifier request #\) = 0 #\;;  The IsProtocolServiceSupported will return true if the specified
...
;  returned, it will be returned in the serviceValue output parameter.
 virtual bool IsProtocolServiceSupported #\( SCSIProtocolFeature feature #\, void * serviceValue #\) = 0 #\;;  The HandleProtocolServiceFeature will return true if the specified feature could
...
;  sent as a parameter, it will be sent in the serviceValue parameter.
 virtual bool HandleProtocolServiceFeature #\( SCSIProtocolFeature feature #\, void * serviceValue #\) = 0 #\; protected #\:;  Reserve space for future expansion.
 struct IOSCSIProtocolInterfaceExpansionData #\{ #\} #\; IOSCSIProtocolInterfaceExpansionData * fIOSCSIProtocolInterfaceReserved #\;;  ------ Power Management Support ------
 thread_call_t fPowerManagementThread #\; IOCommandGate * fCommandGate #\; UInt32 fCurrentPowerState #\; UInt32 fProposedPowerState #\; bool fPowerTransitionInProgress #\; bool fPowerAckInProgress #\; bool fPowerManagementInitialized #\;;  The InitializePowerManagement method is called to initialize power management.
...
;  the device for power managment. That is left to other layers to do.	
 virtual void InitializePowerManagement #\( IOService * provider #\) #\;;  The GetInitialPowerState method is called once, right after InitializePowerManagement()
;  in order to determine what state the device is initially in at startup time (usually
;  the highest power mode).
 virtual UInt32 GetInitialPowerState #\( void #\) = 0 #\;;  The current implementation of the Power Manager requires that leaf nodes
...
;  transitions to complete before processing the finalize.
 virtual bool finalize #\( IOOptionBits options #\) #\;;  The sHandleSetPowerState method is a static function used as C->C++ glue
;  for going behind the command gate.
 static IOReturn sHandleSetPowerState #\( IOSCSIProtocolInterface * self #\, UInt32 powerStateOrdinal #\) #\;;  The sGetPowerTransistionInProgress method is a static function used as
;  C->C++ glue for getting the command gate.
 static bool sGetPowerTransistionInProgress #\( IOSCSIProtocolInterface * self #\) #\;;  The HandleSetPowerState method is called by the glue code and is on the
...
;  variables as necessary without any multi-threading issues.
 virtual void HandleSetPowerState #\( UInt32 powerStateOrdinal #\) #\;;  The sPowerManagement method is a static C-function which is called using
...
;  a state machine used to direct power management.
 static void sPowerManagement #\( thread_call_param_t whichDevice #\) #\;;  The HandlePowerChange method is pure virtual and is left to each protocol or
...
;  own thread of execution and can make synchronous or asynchronous calls.
 virtual void HandlePowerChange #\( void #\) = 0 #\;;  The sHandleCheckPowerState method is a static function used as C->C++ glue
;  for going behind the command gate.
 static void sHandleCheckPowerState #\( IOSCSIProtocolInterface * self #\) #\;;  The HandleCheckPowerState (void) method is on the serialized side of the command
...
;  with the max power state the class registered with.
 virtual void HandleCheckPowerState #\( void #\) = 0 #\;;  The HandleCheckPowerState ( UInt32 maxPowerState ) method is called by
...
;  power state has been achieved.
 void HandleCheckPowerState #\( UInt32 maxPowerState #\) #\;;  The TicklePowerManager ( void ) method is called by CheckPowerState and
;  sends an activity tickle to the power manager so that the idle timer is
;  reset.
 virtual void TicklePowerManager #\( void #\) = 0 #\;;  The TicklePowerManager ( UInt32 maxPowerState ) method is a convenience
...
;  (true if device is in the requested state, false if it is not).
 bool TicklePowerManager #\( UInt32 maxPowerState #\) #\;;  ------ User Client Support ------
 bool fUserClientExclusiveControlled #\; IOService * fUserClient #\;;  The sGetUserClientExclusivityState method is a static function used as C->C++ glue
;  for going behind the command gate.
 static void sGetUserClientExclusivityState #\( IOSCSIProtocolInterface * self #\, bool * state #\) #\; static void sSetUserClientExclusivityState #\( IOSCSIProtocolInterface * self #\, IOReturn * result #\, IOService * userClient #\, bool state #\) #\; virtual bool HandleGetUserClientExclusivityState #\( void #\) #\; virtual IOReturn HandleSetUserClientExclusivityState #\( IOService * userClient #\, bool state #\) #\; public #\:;  ------- SCSI Architecture Model Task Management Functions ------
 OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 1 #\) #\;;  The ABORT TASK function allows an SCSI Application Layer client to request 
 virtual SCSIServiceResponse AbortTask #\( UInt8 theLogicalUnit #\, SCSITaggedTaskIdentifier theTag #\) = 0 #\; OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 2 #\) #\; virtual SCSIServiceResponse AbortTaskSet #\( UInt8 theLogicalUnit #\) = 0 #\; OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 3 #\) #\; virtual SCSIServiceResponse ClearACA #\( UInt8 theLogicalUnit #\) = 0 #\; OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 4 #\) #\; virtual SCSIServiceResponse ClearTaskSet #\( UInt8 theLogicalUnit #\) = 0 #\; OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 5 #\) #\; virtual SCSIServiceResponse LogicalUnitReset #\( UInt8 theLogicalUnit #\) = 0 #\; OSMetaClassDeclareReservedUsed #\( IOSCSIProtocolInterface #\, 6 #\) #\;;  The TARGET RESET management function will cause the Protocol Services Driver
;  to perform the reset
 virtual SCSIServiceResponse TargetReset #\( void #\) = 0 #\; private #\:;  Space reserved for future expansion.
 OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 7 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 8 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 9 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 10 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 11 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 12 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 13 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 14 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 15 #\) #\; OSMetaClassDeclareReservedUnused #\( IOSCSIProtocolInterface #\, 16 #\) #\;
|#
 |#
...
(provide-interface "IOSCSIProtocolInterface")