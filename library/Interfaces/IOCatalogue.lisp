(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOCatalogue.h"
...
(require-interface "libkern/c++/OSCollectionIterator")

(require-interface "libkern/c++/OSArray")

(require-interface "libkern/c++/OSDictionary")

(require-interface "IOKit/IOLocks")

(require-interface "sys/cdefs")

(require-interface "IOKit/IOKitServer")
...
; 
#|
 confused about CLASS IOCatalogue #\: public OSObject #\{ OSDeclareDefaultStructors #\( IOCatalogue #\) private #\: OSCollectionIterator * kernelTables #\; OSArray * array #\; IOLock * lock #\; SInt32 generation #\; IOLock * kld_lock #\; public #\:
; !
;         @function initialize
...
;      
 virtual bool serialize #\( OSSerialize * s #\) const #\; bool serializeData #\( IOOptionBits kind #\, OSSerialize * s #\) const #\;
; !
...
;     
 virtual kern_return_t removeKernelLinker #\( void #\) #\; private #\:
; !
...
; 
(def-mactype :IOKitRelocStart (find-mactype ':KMOD_START_FUNC_T))
; !
;     @function IOKitRelocStop
;     @abstract Deprecated API.
; 
(def-mactype :IOKitRelocStop (find-mactype ':KMOD_STOP_FUNC_T))
(def-mactype :gIOClassKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOProbeScoreKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOCatalogue (find-mactype '(:pointer :IOCatalogue)))

; #endif /* ! _IOKIT_IOCATALOGUE_H */


(provide-interface "IOCatalogue")