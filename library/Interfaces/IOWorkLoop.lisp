(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOWorkLoop.h"
; at Sunday July 2,2006 7:26:46 pm.
; 
...
#|
 confused about CLASS IOWorkLoop #\: public OSObject #\{ OSDeclareDefaultStructors #\( IOWorkLoop #\) public #\:
; !
;     @typedef Action
...
; 
 typedef IOReturn #\( * Action #\) #\( OSObject * target #\, void * arg0 #\, void * arg1 #\, void * arg2 #\, void * arg3 #\) #\; private #\:
...
;     @abstract Static function that calls the $link threadMain function. 
 static void threadMainContinuation #\( #\) #\; protected #\:
...
;     Mutual exlusion lock that used by close and open Gate functions.  
 IORecursiveLock * gateLock #\;                 ; ! @var eventChain Pointer to first Event Source in linked list.  
 IOEventSource * eventChain #\;                 ; ! @var controlG Internal control gate to maintain event system.  
 IOCommandGate * controlG #\;
; ! @var workSpinLock
;     The spin lock that is used to guard the 'workToDo' variable.  
 IOSimpleLock * workToDoLock #\;                ; ! @var workThread Work loop thread.	 
 IOThread workThread #\;
; ! @var workToDo
...
;     @discussion Work loop threads main function.  This function consists of 3 loops: the outermost loop is the semaphore clear and wait loop, the middle loop terminates when there is no more work and the inside loop walks the event list calling the $link checkForWork method in each event source.  If an event source has more work to do then it can set the more flag and the middle loop will repeat.  When no more work is outstanding the outermost will sleep until and event is signaled or the least wakeupTime whichever occurs first.  If the event source does not require the semaphore wait to time out it must set the provided wakeupTime parameter to zero. 
 virtual void threadMain #\( #\) #\; public #\:
; ! @function workLoop
...
;     @discussion For all event sources, ES, for which IODynamicCast(IOInterruptEventSource, ES) is valid,  in $link eventChain call disable() function.	See $link IOEventSource::disable() 
 virtual void disableAllInterrupts #\( #\) const #\; protected #\:;  Internal APIs used by event sources to control the thread
 friend class IOEventSource #\; virtual void signalWorkAvailable #\( #\) #\; virtual void openGate #\( #\) #\; virtual void closeGate #\( #\) #\; virtual bool tryCloseGate #\( #\) #\; virtual int sleepGate #\( void * event #\, UInt32 interuptibleType #\) #\; virtual void wakeupGate #\( void * event #\, bool oneThread #\) #\; public #\:;  methods available in Mac OS X 10.1 or later 
; ! @function runAction
...
; 
 virtual IOReturn runAction #\( Action action #\, OSObject * target #\, void * arg0 = 0 #\, void * arg1 = 0 #\, void * arg2 = 0 #\, void * arg3 = 0 #\) #\; protected #\: OSMetaClassDeclareReservedUsed #\( IOWorkLoop #\, 0 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 1 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 2 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 3 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 4 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 5 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 6 #\) #\; OSMetaClassDeclareReservedUnused #\( IOWorkLoop #\, 7 #\) #\;
|#

; #endif /* !__IOKIT_IOWORKLOOP_H */


(provide-interface "IOWorkLoop")