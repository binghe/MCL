(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:SecIdentitySearch.h"
; at Sunday July 2,2006 7:31:44 pm.
; 
...
(def-mactype :SecIdentitySearchRef (find-mactype '(:pointer :OpaqueSecIdentitySearchRef)))
; !
...
(deftrap-inline "_SecIdentitySearchGetTypeID" 
   (
   )
   :UInt32
() )
...
(deftrap-inline "_SecIdentitySearchCreate" 
   ((keychainOrArray (:pointer :void))
    (keyUsage :UInt32)
    (searchRef (:pointer :SECIDENTITYSEARCHREF))
   )
   :SInt32
() )
; !
...
(deftrap-inline "_SecIdentitySearchCopyNext" 
   ((searchRef (:pointer :OpaqueSecIdentitySearchRef))
    (identity (:pointer :SECIDENTITYREF))
   )
   :SInt32
() )
...
(provide-interface "SecIdentitySearch")