(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOPowerSources.h"
...
(def-mactype :IOPowerSourceCallbackType (find-mactype ':pointer)); (void * context)
...
(deftrap-inline "_IOPSCopyPowerSourcesInfo" 
   (
   )
   (:pointer :void)
() )
...
(deftrap-inline "_IOPSCopyPowerSourcesList" 
   ((blob (:pointer :void))
   )
   (:pointer :__CFArray)
() )
...
(deftrap-inline "_IOPSGetPowerSourceDescription" 
   ((blob (:pointer :void))
    (ps (:pointer :void))
   )
...
(deftrap-inline "_IOPSNotificationCreateRunLoopSource" 
   ((ARG2 :pointer)
    (ARGH :pointer)
   )
...
(provide-interface "IOPowerSources")