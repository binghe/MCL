(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:Scrap.h"
...
(def-mactype :ScrapFlavorType (find-mactype ':FourCharCode))
; 
...
(defconstant $kScrapFlavorTypePicture :|PICT|)  ;  contents of a PicHandle

(defconstant $kScrapFlavorTypeText :|TEXT|)     ;  stream of characters

(defconstant $kScrapFlavorTypeTextStyle :|styl|);  see TEGetStyleScrapHandle

(defconstant $kScrapFlavorTypeMovie :|moov|)    ;  reference to a movie

(defconstant $kScrapFlavorTypeSound :|snd |)    ;  see SndRecord and SndPlay

(defconstant $kScrapFlavorTypeUnicode :|utxt|)  ;  stream of UTF16 characters

(defconstant $kScrapFlavorTypeUnicodeStyle :|ustl|);  ATSUI defines; Textension uses
...
;     ________________________________________________________________
; 
(defrecord ScrapStuff
   (scrapSize :SInt32)
   (scrapHandle :Handle)
   (scrapCount :SInt16)
   (scrapState :SInt16)
   (scrapName (:pointer :UInt8))
)

;type name? (def-mactype :ScrapStuff (find-mactype ':ScrapStuff))

(def-mactype :PScrapStuff (find-mactype '(:pointer :ScrapStuff)))

(def-mactype :ScrapStuffPtr (find-mactype '(:pointer :ScrapStuff)))
 |#
...
(defconstant $kScrapFlavorSizeUnknown -1)
; 
...
(defconstant $kScrapReservedFlavorType :|srft|)
; 
...
(defconstant $kScrapFlavorMaskNone 0)
(defconstant $kScrapFlavorMaskSenderOnly 1)
(defconstant $kScrapFlavorMaskTranslated 2)

(def-mactype :ScrapFlavorFlags (find-mactype ':UInt32))
; 
;     ScrapFlavorInfo describes a single flavor within
;     a scrap.
; 
(defrecord ScrapFlavorInfo
   (flavorType :FourCharCode)
   (flavorFlags :UInt32)
)

;type name? (%define-record :ScrapFlavorInfo (find-record-descriptor ':ScrapFlavorInfo))

(def-mactype :ScrapRef (find-mactype '(:pointer :OpaqueScrapRef)))
; 
...
(defconstant $kScrapGetNamedScrap 0)            ;  get current named scrap without bumping
;  acquire the named scrap, bumping and clearing

(defconstant $kScrapClearNamedScrap 1)
; 
...
(deftrap-inline "_GetScrapByName" 
   ((name (:pointer :__CFString))
    (options :UInt32)
    (scrap (:pointer :SCRAPREF))
...
(deftrap-inline "_GetCurrentScrap" 
   ((scrap (:pointer :SCRAPREF))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_GetScrapFlavorFlags" 
   ((scrap (:pointer :OpaqueScrapRef))
    (flavorType :FourCharCode)
    (flavorFlags (:pointer :SCRAPFLAVORFLAGS))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_GetScrapFlavorSize" 
   ((scrap (:pointer :OpaqueScrapRef))
    (flavorType :FourCharCode)
    (byteCount (:pointer :SIZE))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_GetScrapFlavorData" 
   ((scrap (:pointer :OpaqueScrapRef))
    (flavorType :FourCharCode)
    (byteCount (:pointer :SIZE))
    (destination :pointer)
...
(deftrap-inline "_ClearCurrentScrap" 
   (
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_ClearScrap" 
   ((inOutScrap (:pointer :SCRAPREF))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_1_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_PutScrapFlavor" 
   ((scrap (:pointer :OpaqueScrapRef))
    (flavorType :FourCharCode)
    (flavorFlags :UInt32)
    (flavorSize :signed-long)
    (flavorData :pointer)                       ;  can be NULL 
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(def-mactype :ScrapPromiseKeeperProcPtr (find-mactype ':pointer)); (ScrapRef scrap , ScrapFlavorType flavorType , void * userData)

(def-mactype :ScrapPromiseKeeperUPP (find-mactype '(:pointer :OpaqueScrapPromiseKeeperProcPtr)))
; 
...
(deftrap-inline "_NewScrapPromiseKeeperUPP" 
   ((userRoutine :pointer)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   (:pointer :OpaqueScrapPromiseKeeperProcPtr)
() )
; 
...
(deftrap-inline "_DisposeScrapPromiseKeeperUPP" 
   ((userUPP (:pointer :OpaqueScrapPromiseKeeperProcPtr))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   nil
() )
; 
...
(deftrap-inline "_InvokeScrapPromiseKeeperUPP" 
   ((scrap (:pointer :OpaqueScrapRef))
    (flavorType :FourCharCode)
    (userData :pointer)
    (userUPP (:pointer :OpaqueScrapPromiseKeeperProcPtr))
...
(deftrap-inline "_SetScrapPromiseKeeper" 
   ((scrap (:pointer :OpaqueScrapRef))
    (upp (:pointer :OpaqueScrapPromiseKeeperProcPtr))
    (userData :pointer)
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
...
(deftrap-inline "_GetScrapFlavorCount" 
   ((scrap (:pointer :OpaqueScrapRef))
    (infoCount (:pointer :UInt32))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_GetScrapFlavorInfoList" 
   ((scrap (:pointer :OpaqueScrapRef))
    (infoCount (:pointer :UInt32))
    (info (:pointer :ScrapFlavorInfo))
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
; 
...
(deftrap-inline "_CallInScrapPromises" 
   (
   )                                            ; AVAILABLE_MAC_OS_X_VERSION_10_0_AND_LATER
   :OSStatus
() )
