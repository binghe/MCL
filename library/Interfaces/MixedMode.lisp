(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:MixedMode.h"
...
(defconstant $kCurrentMixedModeStateRecord 1)
;  Calling Conventions 

(def-mactype :CallingConventionType (find-mactype ':UInt16))
...
(defconstant $kRegisterA6 14)                   ;  A7 is the same as the PowerPC SP 
...
(def-mactype :registerSelectorType (find-mactype ':UInt16))
...
(def-mactype :ProcInfoType (find-mactype ':UInt32))
;  Routine Flag Bits 

(def-mactype :RoutineFlagsType (find-mactype ':UInt16))

(defconstant $kProcDescriptorIsAbsolute 0)
(defconstant $kProcDescriptorIsRelative 1)

(defconstant $kFragmentIsPrepared 0)
(defconstant $kFragmentNeedsPreparing 2)

(defconstant $kUseCurrentISA 0)
(defconstant $kUseNativeISA 4)

(defconstant $kPassSelector 0)
(defconstant $kDontPassSelector 8)

(defconstant $kRoutineIsNotDispatchedDefaultRoutine 0)
(defconstant $kRoutineIsDispatchedDefaultRoutine 16)

(defconstant $kProcDescriptorIsProcPtr 0)
(defconstant $kProcDescriptorIsIndex 32)
(defrecord RoutineRecord
   (procInfo :UInt32)                           ;  calling conventions 
   (reserved1 :SInt8)                           ;  Must be 0 
   (ISA :SInt8)                                 ;  Instruction Set Architecture 
   (routineFlags :UInt16)                       ;  Flags for each routine 
...
;type name? (%define-record :RoutineRecord (find-record-descriptor ':RoutineRecord))
...
(def-mactype :RDFlagsType (find-mactype ':UInt8))

(defconstant $kSelectorsAreNotIndexable 0)
(defconstant $kSelectorsAreIndexable 1)
...
(defrecord RoutineDescriptor
   ...
   (routineDescriptorFlags :UInt8)              ;  Routine Descriptor Flags 
...
;type name? (%define-record :RoutineDescriptor (find-record-descriptor ':RoutineDescriptor))
...
(defrecord MixedModeStateRecord
   (state1 :UInt32)
   (state2 :UInt32)
   (state3 :UInt32)
   (state4 :UInt32)
...
;type name? (%define-record :MixedModeStateRecord (find-record-descriptor ':MixedModeStateRecord))
...
; #define BUILD_ROUTINE_DESCRIPTOR(procInfo, procedure)                                   {                                                                                       _MixedModeMagic,                            /* Mixed Mode A-Trap */                 kRoutineDescriptorVersion,                  /* version */                           kSelectorsAreNotIndexable,                  /* RD Flags - not dispatched */         0,                                          /* reserved 1 */                        0,                                          /* reserved 2 */                        0,                                          /* selector info */                     0,                                          /* number of routines */                {                                           /* Its an array */                         {                                       /* Its a struct */                             (procInfo),                         /* the ProcInfo */                              0,                                  /* reserved */                                  GetCurrentArchitecture(),           /* ISA and RTA */                               kProcDescriptorIsAbsolute |         /* Flags - its absolute addr */                kFragmentIsPrepared |               /* Its prepared */                             kUseNativeISA,                      /* Always use native ISA */                     (ProcPtr)(procedure),               /* the procedure */                             0,                                  /* reserved */                                  0                                   /* Not dispatched */                        }                                                                               }                                                                               }
;  a macro which creates a static instance of a fat routine descriptor 
; #define BUILD_FAT_ROUTINE_DESCRIPTOR(m68kProcInfo, m68kProcPtr, powerPCProcInfo, powerPCProcPtr)      {                                                                                       _MixedModeMagic,                            /* Mixed Mode A-Trap */                 kRoutineDescriptorVersion,                  /* version */                           kSelectorsAreNotIndexable,                  /* RD Flags - not dispatched */         0,                                          /* reserved */                          0,                                          /* reserved */                          0,                                          /* reserved */                          1,                                          /* Array count */                       {                                           /* Its an array */                         {                                       /* Its a struct */                             (m68kProcInfo),                     /* the ProcInfo */                              0,                                  /* reserved */                                  kM68kISA |                          /* ISA */                                       kOld68kRTA,                         /* RTA */                                       kProcDescriptorIsAbsolute |         /* Flags - its absolute addr */                kUseCurrentISA,                     /* Use current ISA */                           (ProcPtr)(m68kProcPtr),             /* the procedure */                             0,                                  /* reserved */                                  0,                                  /* reserved */                              },                                                                                  {                                       /* Its a struct */                             (powerPCProcInfo),                  /* the ProcInfo */                              0,                                  /* reserved */                                  GetCurrentArchitecture(),           /* ISA and RTA */                               kProcDescriptorIsAbsolute |         /* Flags - its absolute addr */                kFragmentIsPrepared |               /* Its prepared */                             kUseCurrentISA,                     /* Always use current ISA */                    (ProcPtr)(powerPCProcPtr),          /* the procedure */                             0,                                  /* reserved */                                  0                                   /* reserved */                              }                                                                               }                                                                               }
 |#
...
(defconstant $kCallingConventionMask 15)        ;  Result Offsets 

(defconstant $kResultSizeWidth 2)
(defconstant $kResultSizePhase 4)
(defconstant $kResultSizeMask 48)               ;  Parameter offsets & widths 

(defconstant $kStackParameterWidth 2)
(defconstant $kStackParameterPhase 6)
(defconstant $kStackParameterMask #xFFFFFFC0)   ;  Register Result Location offsets & widths 

(defconstant $kRegisterResultLocationWidth 5)
(defconstant $kRegisterResultLocationPhase 6)   ;  Register Parameter offsets & widths 

(defconstant $kRegisterParameterWidth 5)
(defconstant $kRegisterParameterPhase 11)
...
(defconstant $kRegisterParameterWhichPhase 2)
(defconstant $kRegisterParameterWhichWidth 3)   ;  Dispatched Stack Routine Selector offsets & widths 

(defconstant $kDispatchedSelectorSizeWidth 2)
(defconstant $kDispatchedSelectorSizePhase 6)   ;  Dispatched Stack Routine Parameter offsets 

(defconstant $kDispatchedParameterPhase 8)      ;  Special Case offsets & widths 

(defconstant $kSpecialCaseSelectorWidth 6)
(defconstant $kSpecialCaseSelectorPhase 4)
(defconstant $kSpecialCaseSelectorMask #x3F0)

(defconstant $kSpecialCase 15)                  ;  (CallingConventionType) 
