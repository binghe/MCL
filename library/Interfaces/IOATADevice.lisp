(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOATADevice.h"
...
(require-interface "IOATADevConfig")
...
; 
#|
 confused about CLASS IOATADevice #\: public IOService #\{ OSDeclareDefaultStructors #\( IOATADevice #\) public #\:;  
; !@function getUnitID
...
; 	
 virtual IOReturn provideConfig #\( IOATADevConfig * configRequest #\) #\;;  Submit IO requests 
; !
...
; 	
 virtual void freeCommand #\( IOATACommand * inCommand #\) #\;;  matching stuff for IOBSDInit and so on.	
...
; 	@abstract matching stuff for IOBSDInit and so on.	
; 	
 virtual bool matchPropertyTable #\( OSDictionary * table #\, SInt32 * score #\) #\;;  called by controllers when they need to send a message to client drivers.
; !
; 	@function notifyEvent
; 	@abstract called by controllers when they need to send a message to client (disk) drivers.
; 	
 virtual void notifyEvent #\( UInt32 event #\) #\; protected #\: IOATAController * _provider #\; ataUnitID _unitNumber #\;;  0 - master, 1 - slave, -1 = not Valid
