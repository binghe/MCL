(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOATADevConfig.h"
; at Sunday July 2,2006 7:26:44 pm.
; 
...
(require-interface "IOATABusInfo")
; !
...
; 
#|
 confused about CLASS IOATADevConfig #\: public OSObject #\{ OSDeclareDefaultStructors #\( IOATADevConfig #\) public #\:
...
; 	
 IOReturn initWithBestSelection #\( const UInt16 * identifyData #\, IOATABusInfo * busInfo #\) #\;;  intitialize with the 512 byte data from an ATA device identify command
...
; 	
 atapiConfig getPacketConfig #\( void #\) #\;   ;  The following Mode accessors use bit significance to indicate a selected mode or supported modes(s)
;  ie, 00000001b indicates Mode-0, 00000010b indicates mode 1, etc.
;  Selected mode is indicated by a single set bit. No bit set indicates no mode in that class is selected.
;  ie, a bus will support multiple possible modes, but will only have one mode selected at that time.
; !@function setPIOMode
...
; 	
 UInt8 getUltraMode #\( void #\) #\;            ;  The following cycle time accessors report cycle times in nanoseconds.
...
; 	
 UInt16 getDMACycleTime #\( void #\) #\;        ;  Ultra ATA defines cycle times a device must meet to comply with standards.
...
; 	
 virtual UInt8 bitSigToNumeric #\( UInt16 binary #\) #\; protected #\: atapiConfig _atapiIRQForPacket #\;;  enum for bits 5 and 6 of word zero of 
...
; 	kATAPIUnknown   = 0x11   - reserved as of ATA/ATAPI-5
 ...
 IOReturn _AssignPIOData #\( const UInt16 * identifyData #\) #\; IOReturn _AssignDMAData #\( const UInt16 * identifyData #\) #\; IOReturn _AssignUltraData #\( const UInt16 * identifyData #\) #\; UInt8 _MostSignificantBit #\( UInt8 inByte #\) #\; virtual bool init #\( #\) #\; protected #\:
...
(provide-interface "IOATADevConfig")