(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOUSBDevice.h"
; at Sunday July 2,2006 7:29:59 pm.
; 
...
; 
(defconstant $kAllowConfigValueOfZero "kAllowZeroConfigValue")
; #define kAllowConfigValueOfZero		"kAllowZeroConfigValue"
(defconstant $kAllowNumConfigsOfZero "kAllowZeroNumConfigs")
...
; 
#|
 confused about CLASS IOUSBDevice #\: public IOUSBNub #\{ friend class IOUSBController #\; friend class IOUSBInterface #\; friend class IOUSBPipe #\; OSDeclareDefaultStructors #\( IOUSBDevice #\) protected #\: USBDeviceAddress _address #\; IOUSBController * _controller #\; IOUSBPipe * _pipeZero #\; IOUSBDeviceDescriptor _descriptor #\; UInt32 _busPowerAvailable #\; UInt8 _speed #\; IOUSBEndpointDescriptor _endpointZero #\;;  Fake ep for control pipe
 void * _port #\;                               ;  Obsolete, do not use
 IOBufferMemoryDescriptor ** _configList #\; IOUSBInterface ** _interfaceList #\; UInt8 _currentConfigValue #\; UInt8 _numInterfaces #\; struct ExpansionData #\{ UInt32 _portNumber #\; thread_call_t _doPortResetThread #\; IOUSBDevice * _usbPlaneParent #\; bool _portResetThreadActive #\; bool _allowConfigValueOfZero #\; thread_call_t _doPortSuspendThread #\; bool _portSuspendThreadActive #\; thread_call_t _doPortReEnumerateThread #\; bool _resetInProgress #\; bool _portHasBeenReset #\; bool _deviceterminating #\; IORecursiveLock * _getConfigLock #\; bool _doneWaiting #\;;  Obsolete
 bool _notifiedWhileBooting #\;                 ;  Obsolete
 IOWorkLoop * _workLoop #\; IOTimerEventSource * _notifierHandlerTimer #\; UInt32 _notificationType #\; bool _suspendInProgress #\; bool _portHasBeenSuspended #\; #\} #\; ExpansionData * _expansionData #\; virtual void free #\( #\) #\; const IOUSBConfigurationDescriptor * FindConfig #\( UInt8 configValue #\, UInt8 * configIndex = 0 #\) #\; virtual IOUSBInterface * GetInterface #\( const IOUSBInterfaceDescriptor * interface #\) #\; public #\: virtual IOReturn SetFeature #\( UInt8 feature #\) #\; protected #\: virtual IOReturn GetConfigDescriptor #\( UInt8 configIndex #\, void * data #\, UInt32 len #\) #\; virtual IOReturn GetDeviceDescriptor #\( IOUSBDeviceDescriptor * desc #\, UInt32 size #\) #\; virtual bool init #\( USBDeviceAddress deviceAddress #\, UInt32 powerAvaiable #\, UInt8 speed #\, UInt8 maxPacketSize #\) #\; virtual bool matchPropertyTable #\( OSDictionary * table #\, SInt32 * score #\) #\; public #\: virtual void SetProperties #\( #\) #\; static IOUSBDevice * NewDevice #\( void #\) #\;;  IOService methods
 virtual bool attach #\( IOService * provider #\) #\; virtual bool start #\( IOService * provider #\) #\; virtual void stop #\( IOService * provider #\) #\; virtual bool finalize #\( IOOptionBits options #\) #\; virtual IOReturn message #\( UInt32 type #\, IOService * provider #\, void * argument = 0 #\) #\; virtual bool willTerminate #\( IOService * provider #\, IOOptionBits options #\) #\; virtual bool didTerminate #\( IOService * provider #\, IOOptionBits options #\, bool * defer #\) #\; virtual void SetPort #\( void * port #\) #\;;  Obsolete, do NOT use
; !
...
;     
 virtual OSIterator * CreateInterfaceIterator #\( IOUSBFindInterfaceRequest * request #\) #\;;  Get pointer to full config info (cached in device, don't free returned pointer)
; !
...
;     
 virtual IOReturn SetConfiguration #\( IOService * forClient #\, UInt8 configValue #\, bool startInterfaceMatching = true #\) #\;;  Access to addressing and cached info
; !
;         @function GetAddress
...
;     
 virtual IOUSBPipe * MakePipe #\( const IOUSBEndpointDescriptor * ep #\) #\;;  this method is deprecated. use the other DeviceRequest methods
 virtual IOReturn DeviceRequest #\( IOUSBDevRequest * request #\, IOUSBCompletion * completion = 0 #\) #\;;  Same but with a memory descriptor
...
;     
 virtual UInt32 GetChildLocationID #\( UInt32 parentLocationID #\, int port #\) #\; virtual const IOUSBDescriptorHeader * FindNextDescriptor #\( const void * cur #\, UInt8 descType #\) #\; virtual void DisplayNotEnoughPowerNotice #\( #\) #\;;  this is a non-virtual function so that we don't have to take up a binary compatibility slot.
 UInt16 GetbcdUSB #\( void #\) #\; UInt8 GetProtocol #\( void #\) #\; OSMetaClassDeclareReservedUsed #\( IOUSBDevice #\, 0 #\) #\;
; !
...
;     
 virtual IOReturn DeviceRequest #\( IOUSBDevRequest * request #\, UInt32 noDataTimeout #\, UInt32 completionTimeout #\, IOUSBCompletion * completion = 0 #\) #\;;  Same but with a memory descriptor
 OSMetaClassDeclareReservedUsed #\( IOUSBDevice #\, 1 #\) #\; virtual IOReturn DeviceRequest #\( IOUSBDevRequestDesc * request #\, UInt32 noDataTimeout #\, UInt32 completionTimeout #\, IOUSBCompletion * completion = 0 #\) #\; OSMetaClassDeclareReservedUsed #\( IOUSBDevice #\, 2 #\) #\;
; !
...
;     
 virtual IOReturn SuspendDevice #\( bool suspend #\) #\; OSMetaClassDeclareReservedUsed #\( IOUSBDevice #\, 3 #\) #\;
; !
...
;     
 virtual IOReturn ReEnumerateDevice #\( UInt32 options #\) #\; OSMetaClassDeclareReservedUsed #\( IOUSBDevice #\, 4 #\) #\;
; !
...
;      
 virtual void DisplayUserNotification #\( UInt32 notificationType #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 5 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 6 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 7 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 8 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 9 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 10 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 11 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 12 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 13 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 14 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 15 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 16 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 17 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 18 #\) #\; OSMetaClassDeclareReservedUnused #\( IOUSBDevice #\, 19 #\) #\; private #\: static void ProcessPortResetEntry #\( OSObject * target #\) #\; void ProcessPortReset #\( void #\) #\; void TerminateInterfaces #\( void #\) #\; static void ProcessPortSuspendEntry #\( OSObject * target #\, thread_call_param_t suspend #\) #\; void ProcessPortSuspend #\( bool suspend #\) #\; static void ProcessPortReEnumerateEntry #\( OSObject * target #\, thread_call_param_t options #\) #\; void ProcessPortReEnumerate #\( UInt32 options #\) #\; static void DisplayUserNotificationForDeviceEntry #\( OSObject * owner #\, IOTimerEventSource * sender #\) #\; void DisplayUserNotificationForDevice #\( #\) #\;
|#

; #endif /* _IOKIT_IOUSBDEVICE_H */


(provide-interface "IOUSBDevice")