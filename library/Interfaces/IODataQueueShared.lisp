(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IODataQueueShared.h"
...
;  
(defrecord _IODataQueueEntry
   (size :UInt32)
   (data :pointer)
)
(%define-record :IODataQueueEntry (find-record-descriptor :_IODATAQUEUEENTRY))
; !
...
;  
(defrecord _IODataQueueMemory
   (queueSize :UInt32)
   (head :UInt32)
   (tail :UInt32)
   (queue (:array :_IODATAQUEUEENTRY 1))
)
(%define-record :IODataQueueMemory (find-record-descriptor :_IODATAQUEUEMEMORY))
; !
...
;  
(defconstant $DATA_QUEUE_ENTRY_HEADER_SIZE 4)
