(in-package :TRAPS)
; Generated from #P"macintosh-hd:hd3:CInterface Translator:Source Interfaces:IOService.h"
; at Sunday July 2,2006 7:25:41 pm.
; 
...
(require-interface "IOKit/IODeviceMemory")

(require-interface "IOKit/IONotifier")

(require-interface "IOKit/IOLocks")

(require-interface "IOKit/IOKitDebug")
...
(require-interface "IOKit/pwr_mgt/IOPMpowerState")

(require-interface "IOKit/IOServicePM")
; Gag EXTERN "C"  {

(require-interface "kern/thread_call")

(defconstant $kIODefaultProbeScore 0)
;  masks for getState()

(defconstant $kIOServiceInactiveState 1)
(defconstant $kIOServiceRegisteredState 2)
(defconstant $kIOServiceMatchedState 4)
(defconstant $kIOServiceFirstPublishState 8)
(defconstant $kIOServiceFirstMatchState 16)
;  options for registerService()

(defconstant $kIOServiceExclusive 1)            ;  options for terminate()

(defconstant $kIOServiceRequired 1)
(defconstant $kIOServiceTerminate 4)            ;  options for registerService() & terminate()

(defconstant $kIOServiceSynchronous 2)          ;  options for registerService()

(defconstant $kIOServiceAsynchronous 8)
;  options for open()

(defconstant $kIOServiceSeize 1)
(defconstant $kIOServiceFamilyOpenOptions #xFFFF0000)
;  options for close()

(defconstant $kIOServiceFamilyCloseOptions #xFFFF0000)

(def-mactype :IONotificationRef (find-mactype '(:pointer :void)))
(def-mactype :gIOServicePlane (find-mactype '(:pointer :IORegistryPlane)))
(def-mactype :gIOPowerPlane (find-mactype '(:pointer :IORegistryPlane)))
(def-mactype :gIOResourcesKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOResourceMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOProviderClassKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIONameMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIONameMatchedKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOPropertyMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOLocationMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOParentMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOPathMatchKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOMatchCategoryKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIODefaultMatchCategoryKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOMatchedServiceCountKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOUserClientClassKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOKitDebugKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOServiceKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOCommandPoolSizeKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOPublishNotification (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOFirstPublishNotification (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOMatchedNotification (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOFirstMatchNotification (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOTerminatedNotification (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOGeneralInterest (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOBusyInterest (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOOpenInterest (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOAppPowerStateInterest (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOPriorityPowerStateInterest (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIODeviceMemoryKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOInterruptControllersKey (find-mactype '(:pointer :OSSymbol)))
(def-mactype :gIOInterruptSpecifiersKey (find-mactype '(:pointer :OSSymbol)))

(deftrap-inline "_IOServiceOrdering" 
   ((inObj1 (:pointer :osmetaclassbase))
    (inObj2 (:pointer :osmetaclassbase))
    (ref :pointer)
   )
   :SInt32
() )

(def-mactype :IOInterruptAction (find-mactype ':pointer)); (OSObject * target , void * refCon , IOService * nub , int source)
...
(def-mactype :IOServiceNotificationHandler (find-mactype ':pointer)); (void * target , void * refCon , IOService * newService)
...
(def-mactype :IOServiceInterestHandler (find-mactype ':pointer)); (void * target , void * refCon , UInt32 messageType , IOService * provider , void * messageArgument , vm_size_t argSize)

(def-mactype :IOServiceApplierFunction (find-mactype ':pointer)); (IOService * service , void * context)

(def-mactype :OSObjectApplierFunction (find-mactype ':pointer)); (OSObject * object , void * context)
...
; 
#|
 confused about CLASS IOService #\: public IORegistryEntry #\{ OSDeclareDefaultStructors #\( IOService #\) protected #\:
...
;     Reserved for future use.  (Internal use only)  
 ExpansionData * reserved #\; private #\: IOService * __provider #\; SInt32 __providerGeneration #\; IOService * __owner #\; IOOptionBits __state #\[ 2 #\] #\; IOOptionBits __reserved #\[ 4 #\] #\;;  pointer to private instance variables for power management
 IOPMpriv * priv #\; protected #\:              ;  TRUE once PMinit has been called
 bool initialized #\; public #\:                ;  pointer to protected instance variables for power management
 IOPMprot * pm_vars #\; public #\:              ;  methods available in Mac OS X 10.1 or later 
...
;     @result Returns true if the IOService has been terminated. 
 bool isInactive #\( void #\) const #\;         ;  Stack creation 
...
;     @discussion The termination process for an IOService (the provider) will call stop in each of its clients, after they have closed the provider if they had it open, or immediately on termination. 
 virtual void stop #\( IOService * provider #\) #\;;  Open / Close 
; ! @function open
...
;     @param forClient If non-zero, isOpen returns the open state for that client. If zero is passed, isOpen returns the open state for all clients.
;     @result Returns true if the specific, or any, client has the IOService open. 
 virtual bool handleIsOpen #\( const IOService * forClient #\) const #\;;  Stacking change 
; ! @function terminate
...
;     @result result of the terminate request on the client. 
 virtual bool terminateClient #\( IOService * client #\, IOOptionBits options #\) #\;;  Busy state indicates discovery, matching or termination is in progress 
...
;     @result Returns an error code if mach synchronization primitives fail, kIOReturnTimeout, or kIOReturnSuccess. 
 virtual IOReturn waitQuiet #\( mach_timespec_t * timeout = 0 #\) #\;;  Matching 
...
;     @result Returns false if the family considers the matching dictionary does not match in properties it understands, true otherwise. 
 virtual bool matchPropertyTable #\( OSDictionary * table #\, SInt32 * score #\) #\; virtual bool matchPropertyTable #\( OSDictionary * table #\) #\;
...
;     @result Returns the IOService instance to be used for location matching. 
 virtual IOService * matchLocation #\( IOService * client #\) #\;;  Resource service 
...
;     @param The object to be published. 
 static void publishResource #\( const char * key #\, OSObject * value = 0 #\) #\; virtual bool addNeededResource #\( const char * key #\) #\;;  Notifications 
...
;     @result The location matching dictionary created is returned on success, or zero on failure. 
 static OSDictionary * addLocation #\( OSDictionary * table #\) #\;;  Helpers for matching dictionaries. 
...
;     @result if compareProperty returns true for each key in the collection, success is return else failure. 
 virtual bool compareProperties #\( OSDictionary * matching #\, OSCollection * keys #\) #\;;  Client / provider accessors 
; ! @function attach
...
;     @result Return an IOReturn code, kIOReturnSuccess if the function was successfully executed, kIOReturnUnsupported if a service to execute the function could not be found.  Other return codes may be returned by the function.
 virtual IOReturn callPlatformFunction #\( const OSSymbol * functionName #\, bool waitForFunction #\, void * param1 #\, void * param2 #\, void * param3 #\, void * param4 #\) #\; virtual IOReturn callPlatformFunction #\( const char * functionName #\, bool waitForFunction #\, void * param1 #\, void * param2 #\, void * param3 #\, void * param4 #\) #\;;  Some accessors 
...
;     @result A pointer to the IOResources instance. It should not be released by the caller. 
 static IOService * getResourceService #\( void #\) #\;;  Allocate resources for a matched service 
...
;     @param array An OSArray of IODeviceMemory objects, or zero if none are available. The array will be retained by the object. 
 virtual void setDeviceMemory #\( OSArray * array #\) #\;;  Interrupt accessors 
...
;     @result An IOReturn code. 
 virtual IOReturn requestProbe #\( IOOptionBits options #\) #\;;  Generic API for non-data-path upstream calls 
; ! @function message
...
;     @result Any non-kIOReturnSuccess return codes returned by the clients, or kIOReturnSuccess if all return kIOReturnSuccess. 
 virtual IOReturn messageClients #\( UInt32 type #\, void * argument = 0 #\, vm_size_t argSize = 0 #\) #\; virtual IONotifier * registerInterest #\( const OSSymbol * typeOfInterest #\, IOServiceInterestHandler handler #\, void * target #\, void * ref = 0 #\) #\; virtual void applyToProviders #\( IOServiceApplierFunction applier #\, void * context #\) #\; virtual void applyToClients #\( IOServiceApplierFunction applier #\, void * context #\) #\; virtual void applyToInterested #\( const OSSymbol * typeOfInterest #\, OSObjectApplierFunction applier #\, void * context #\) #\; virtual IOReturn acknowledgeNotification #\( IONotificationRef notification #\, IOOptionBits response #\) #\;;  User client create 
...
;     @result A return code to be passed back to the caller of IOServiceOpen. 
 virtual IOReturn newUserClient #\( task_t owningTask #\, void * securityID #\, UInt32 type #\, OSDictionary * properties #\, IOUserClient ** handler #\) #\; virtual IOReturn newUserClient #\( task_t owningTask #\, void * securityID #\, UInt32 type #\, IOUserClient ** handler #\) #\;;  Return code utilities 
...
;             
 void settleTimerExpired #\( void #\) #\; IOReturn serializedAllowPowerChange2 #\( unsigned long #\) #\; IOReturn serializedCancelPowerChange2 #\( unsigned long #\) #\;;  implemented by power-controlling driver...
...
;         
 virtual IOReturn newTemperature #\( long currentTemp #\, IOService * whichZone #\) #\; virtual bool askChangeDown #\( unsigned long #\) #\; virtual bool tellChangeDown #\( unsigned long #\) #\; bool tellChangeDown1 #\( unsigned long #\) #\; bool tellChangeDown2 #\( unsigned long #\) #\; virtual void tellNoChangeDown #\( unsigned long #\) #\; virtual void tellChangeUp #\( unsigned long #\) #\; virtual IOReturn allowPowerChange #\( unsigned long refcon #\) #\; virtual IOReturn cancelPowerChange #\( unsigned long refcon #\) #\;;  ...implemented by power-controlling driver
 protected #\:
